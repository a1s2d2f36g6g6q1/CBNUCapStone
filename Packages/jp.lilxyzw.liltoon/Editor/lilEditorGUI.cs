...
namespace lilToon
{
    public class lilEditorGUI
    {
        ...
        public static GUIStyle customToggleFont = new();
        public static GUIStyle wrapLabel = new();
        public static GUIStyle boldLabel = new();
        public static GUIStyle foldout = new();
        public static GUIStyle middleButton = new() { alignment = TextAnchor.MiddleCenter };
        ...
        public static void DrawWebButton(string text, string URL)
        {
            ...
            if (GUI.Button(position, icon, style)) Application.OpenURL(URL);
        }
        ...
        public static bool DrawSimpleFoldout(MaterialEditor materialEditor, GUIContent guiContent,
            MaterialProperty textureName, MaterialProperty rgba, bool condition, bool isCustomEditor = true)
        {
            EditorGUI.indentLevel++;
        ...
        public static bool DrawSimpleFoldout(MaterialEditor materialEditor, GUIContent guiContent,
            MaterialProperty textureName, bool condition, bool isCustomEditor = true)
        {
            EditorGUI.indentLevel++;
        ...
        public static void InitializeGUIStyles()
        {
            ...
            {
                boxOuter.normal.background =
                    (Texture2D)EditorGUIUtility.Load(lilDirectoryManager.GetGUIBoxOutDarkPath());
                boxInnerHalf.normal.background =
                    (Texture2D)EditorGUIUtility.Load(lilDirectoryManager.GetGUIBoxInHalfDarkPath());
                boxInner.normal.background =
                    (Texture2D)EditorGUIUtility.Load(lilDirectoryManager.GetGUIBoxInDarkPath());
                customBox.normal.background =
                    (Texture2D)EditorGUIUtility.Load(lilDirectoryManager.GetGUICustomBoxDarkPath());
                customToggleFont = EditorStyles.label;
            }
            else
            {
                boxOuter.normal.background =
                    (Texture2D)EditorGUIUtility.Load(lilDirectoryManager.GetGUIBoxOutLightPath());
                boxInnerHalf.normal.background =
                    (Texture2D)EditorGUIUtility.Load(lilDirectoryManager.GetGUIBoxInHalfLightPath());
                boxInner.normal.background =
                    (Texture2D)EditorGUIUtility.Load(lilDirectoryManager.GetGUIBoxInLightPath());
                customBox.normal.background =
                    (Texture2D)EditorGUIUtility.Load(lilDirectoryManager.GetGUICustomBoxLightPath());
        ...
        public static bool AutoFixHelpBox(string message)
        {
            ...
            var pressed = GUILayout.Button(GetLoc("sFixNow"));
        ...
        public static void LocalizedProperty(MaterialEditor materialEditor, MaterialProperty prop, string label,
            bool shouldCheck = true)
        {
            if (!shouldCheck || CheckPropertyToDraw(prop))
            {
        ...
        public static void LocalizedPropertyColorWithAlpha(MaterialEditor materialEditor, MaterialProperty prop,
            bool shouldCheck = true)
        {
            if (!shouldCheck || CheckPropertyToDraw(prop))
            {
        ...
        public static Rect TexturePropertySingleLine(MaterialEditor materialEditor, GUIContent content,
        ...
        public static Rect TexturePropertySingleLine(MaterialEditor materialEditor, GUIContent content,
            MaterialProperty tex, MaterialProperty prop)
        {
            if (Event.current.alt)
            {
        ...
        public static void LocalizedPropertyTexture(MaterialEditor materialEditor, GUIContent content,
            MaterialProperty tex, bool shouldCheck = true)
        {
        ...
        public static void LocalizedPropertyTexture(MaterialEditor materialEditor, GUIContent content,
            MaterialProperty tex, MaterialProperty color, bool shouldCheck = true)
        {
        ...
        public static void LocalizedPropertyTextureWithAlpha(MaterialEditor materialEditor, GUIContent content,
            MaterialProperty tex, MaterialProperty color, bool shouldCheck = true)
        {
            if (!shouldCheck || CheckPropertyToDraw(tex) || CheckPropertyToDraw(color) || CheckPropertyToDraw(content))
            {
        ...
        public static bool CheckPropertyToDraw(MaterialProperty prop)
        {
            return string.IsNullOrEmpty(lilToonInspector.edSet.searchKeyWord) ||
                   lilLanguageManager.GetDisplayName(prop).Contains(lilToonInspector.edSet.searchKeyWord) ||
                   prop.name.Contains(lilToonInspector.edSet.searchKeyWord);
        }

        public static bool CheckPropertyToDraw(params MaterialProperty[] props)
        {
            return string.IsNullOrEmpty(lilToonInspector.edSet.searchKeyWord) || props.Count(prop =>
                lilLanguageManager.GetDisplayName(prop).Contains(lilToonInspector.edSet.searchKeyWord) ||
                prop.name.Contains(lilToonInspector.edSet.searchKeyWord)) > 0;
        }
        ...
        public static bool CheckPropertyToDraw(GUIContent content)
        {
            return string.IsNullOrEmpty(lilToonInspector.edSet.searchKeyWord) ||
                   (!string.IsNullOrEmpty(content.text) &&
                    content.text.Contains(lilToonInspector.edSet.searchKeyWord)) ||
                   (!string.IsNullOrEmpty(content.tooltip) &&
                    content.tooltip.Contains(lilToonInspector.edSet.searchKeyWord));
        }

        public static bool CheckPropertyToDraw(string label)
        {
            return string.IsNullOrEmpty(lilToonInspector.edSet.searchKeyWord) ||
                   label.Contains(lilToonInspector.edSet.searchKeyWord);
        }

        public static bool CheckPropertyToDraw(params string[] labels)
        {
            return string.IsNullOrEmpty(lilToonInspector.edSet.searchKeyWord) || labels.Count(l =>
                lilLanguageManager.GetDisplayName(l).Contains(lilToonInspector.edSet.searchKeyWord)) > 0;
        }
        ...
        public static void UV4Decal(MaterialEditor m_MaterialEditor, MaterialProperty isDecal,
            MaterialProperty isLeftOnly, MaterialProperty isRightOnly, MaterialProperty shouldCopy,
            MaterialProperty shouldFlipMirror, MaterialProperty shouldFlipCopy, MaterialProperty tex,
            MaterialProperty SR, MaterialProperty angle, MaterialProperty decalAnimation,
            MaterialProperty decalSubParam, MaterialProperty uvMode)
        {
            if (!CheckPropertyToDraw(isDecal, isLeftOnly, isRightOnly, shouldCopy, shouldFlipMirror, shouldFlipCopy,
                    tex, SR, angle, decalAnimation, decalSubParam, uvMode)) return;
            ...
            if (isDecal.floatValue == 1.0f)
            {
                ...
                EditorGUI.BeginChangeCheck();
                var mmlabel = Event.current.alt
                    ? isLeftOnly.name + ", " + isRightOnly.name + ", " + shouldFlipMirror.name
                    : GetLoc("sMirrorMode");
                mirrorMode = Popup(mmlabel, mirrorMode, GetLoc("sMirrorModeNormal"), GetLoc("sMirrorModeFlip"),
                    GetLoc("sMirrorModeLeft"), GetLoc("sMirrorModeRight"), GetLoc("sMirrorModeRightFlip"));
                ...
                EditorGUI.BeginChangeCheck();
                var cmlabel = Event.current.alt ? shouldCopy.name + ", " + shouldFlipCopy.name : GetLoc("sCopyMode");
                copyMode = Popup(cmlabel, copyMode, GetLoc("sCopyModeNormal"), GetLoc("sCopyModeSymmetry"),
                    GetLoc("sCopyModeFlip"));
                if (EditorGUI.EndChangeCheck())
                {
                    if (copyMode == 0)
                    {
                ...
                // Load scale & offset
                var scaleX = tex.textureScaleAndOffset.x;
                var scaleY = tex.textureScaleAndOffset.y;
                var posX = tex.textureScaleAndOffset.z;
                var posY = tex.textureScaleAndOffset.w;
                ...
                if (copyMode > 0)
                {
                    ...
                    posX = EditorGUILayout.Slider(Event.current.alt ? tex.name + "_ST.z" : GetLoc("sPositionX"), posX,
                        0.5f, 1.0f);
                }
                else
                {
                ...
                // Draw properties
                posY = EditorGUILayout.Slider(Event.current.alt ? tex.name + "_ST.w" : GetLoc("sPositionY"), posY, 0.0f,
                    1.0f);
                scaleX = EditorGUILayout.Slider(Event.current.alt ? tex.name + "_ST.x" : GetLoc("sScaleX"), scaleX,
                    -1.0f, 1.0f);
                scaleY = EditorGUILayout.Slider(Event.current.alt ? tex.name + "_ST.y" : GetLoc("sScaleY"), scaleY,
                    -1.0f, 1.0f);
                if (EditorGUI.EndChangeCheck())
                {
            ...
            if (Button(GetLoc("sReset")) && EditorUtility.DisplayDialog(GetLoc("sDialogResetUV"),
                    GetLoc("sDialogResetUVMes"), GetLoc("sYes"), GetLoc("sNo")))
            {
        ...
        private static void ScrollAndRotateGUI(MaterialProperty prop)
        {
            ...
            var angle = Radian2Degree(prop.vectorValue.z);
            var rotate = RoundFloat1000000(prop.vectorValue.w / Mathf.PI * 0.5f);
            ...
            EditorGUI.PrefixLabel(labelRect,
                new GUIContent(Event.current.alt ? prop.name + "_ST.xy" : GetLoc("sScroll")));
            ...
            // Scroll
            var vecRect = new Rect(positionVec2.x + labelWidth, positionVec2.y, positionVec2.width - labelWidth,
                positionVec2.height);
            ...
            // Rotate
            rotate = EditorGUI.FloatField(EditorGUILayout.GetControlRect(),
                Event.current.alt ? prop.name + "_ST.w" : GetLoc("sRotate"), rotate);
        ...
        public static void ToneCorrectionGUI(MaterialEditor m_MaterialEditor, MaterialProperty hsvg)
        {
            ...
            // Reset
            if (Button(GetLoc("sReset"))) hsvg.vectorValue = lilConstants.defaultHSVG;
        }
        ...
        public static void DrawVectorAs4Float(MaterialProperty prop, string label0, string label1, string label2,
            string label3)
        {
            if (!CheckPropertyToDraw(prop)) return;
            var param1 = prop.vectorValue.x;
            var param2 = prop.vectorValue.y;
            var param3 = prop.vectorValue.z;
            var param4 = prop.vectorValue.w;
            if (Event.current.alt)
            {
                label0 = prop.name + ".x";
                label1 = prop.name + ".y";
            ...
            if (EditorGUI.EndChangeCheck()) prop.vectorValue = new Vector4(param1, param2, param3, param4);
        }

        public static void DrawColorAsAlpha(MaterialProperty prop, string label)
        {
            if (!CheckPropertyToDraw(label)) return;
            var alpha = prop.colorValue.a;
        ...
        {
            if (CheckPropertyToDraw(tex) && tex.textureValue is Texture2D &&
                !((Texture2D)tex.textureValue).alphaIsTransparency && AutoFixHelpBox(GetLoc("sNotAlphaIsTransparency")))
            {
        ...
        public static void BlendSettingGUI(MaterialEditor m_MaterialEditor, bool isCustomEditor, ref bool isShow,
            string labelName, MaterialProperty srcRGB, MaterialProperty dstRGB, MaterialProperty srcA,
            MaterialProperty dstA, MaterialProperty opRGB, MaterialProperty opA)
        {
        ...
        public static void TextureGUI(MaterialEditor m_MaterialEditor, bool isCustomEditor, ref bool isShow,
            GUIContent guiContent, MaterialProperty textureName)
        {
        ...
        public static void TextureGUI(MaterialEditor m_MaterialEditor, bool isCustomEditor, ref bool isShow,
            GUIContent guiContent, MaterialProperty textureName, MaterialProperty rgba)
        {
        ...
        public static void TextureGUI(MaterialEditor m_MaterialEditor, bool isCustomEditor, ref bool isShow,
            GUIContent guiContent, MaterialProperty textureName, MaterialProperty rgba, MaterialProperty uvMode,
            string sUVMode)
        {
        ...
        public static void TextureGUI(MaterialEditor m_MaterialEditor, bool isCustomEditor, ref bool isShow,
            GUIContent guiContent, MaterialProperty textureName, MaterialProperty rgba, MaterialProperty scrollRotate)
        {
        ...
        public static void TextureGUI(MaterialEditor m_MaterialEditor, bool isCustomEditor, ref bool isShow,
            GUIContent guiContent, MaterialProperty textureName, MaterialProperty rgba, MaterialProperty scrollRotate,
            bool useCustomUV, bool useUVAnimation)
        {
            if (!CheckPropertyToDraw(guiContent) && !CheckPropertyToDraw(textureName, rgba, scrollRotate)) return;
            if (useCustomUV)
            {
        ...
        public static void TextureGUI(MaterialEditor m_MaterialEditor, bool isCustomEditor, ref bool isShow,
            GUIContent guiContent, MaterialProperty textureName, MaterialProperty rgba, MaterialProperty scrollRotate,
            MaterialProperty uvMode, bool useCustomUV, bool useUVAnimation)
        {
            if (!CheckPropertyToDraw(guiContent) &&
                !CheckPropertyToDraw(textureName, rgba, scrollRotate, uvMode)) return;
            if (useCustomUV)
            {
                isShow = DrawSimpleFoldout(m_MaterialEditor, guiContent, textureName, rgba, isShow, isCustomEditor);
                if (isShow)
        ...
        public static void MatCapTextureGUI(MaterialEditor m_MaterialEditor, bool isCustomEditor, ref bool isShow,
            GUIContent guiContent, MaterialProperty textureName, MaterialProperty blendUV1, MaterialProperty zRotCancel,
            MaterialProperty perspective, MaterialProperty vrParallaxStrength)
        {
            if (!CheckPropertyToDraw(guiContent) &&
                !CheckPropertyToDraw(blendUV1, zRotCancel, perspective, vrParallaxStrength)) return;
            isShow = DrawSimpleFoldout(m_MaterialEditor, guiContent, textureName, isShow, isCustomEditor);
            if (isShow)
            {
                EditorGUI.indentLevel++;
        ...
        public static void MatCapTextureGUI(MaterialEditor m_MaterialEditor, bool isCustomEditor, ref bool isShow,
            GUIContent guiContent, MaterialProperty textureName, MaterialProperty rgba, MaterialProperty blendUV1,
            MaterialProperty zRotCancel, MaterialProperty perspective, MaterialProperty vrParallaxStrength)
        {
            if (!CheckPropertyToDraw(guiContent) &&
                !CheckPropertyToDraw(blendUV1, zRotCancel, perspective, vrParallaxStrength)) return;
            isShow = DrawSimpleFoldout(m_MaterialEditor, guiContent, textureName, rgba, isShow, isCustomEditor);
        ...
        public static void ApplyMatCapUVPreset(bool isAngelRing, MaterialProperty blendUV1, MaterialProperty zRotCancel,
            MaterialProperty perspective, MaterialProperty vrParallaxStrength)
        {
            if (isAngelRing)
            {
        ...
        public static void BitEditor8(MaterialProperty prop, string label)
        {
            ...
            b[7] = EditorGUI.Toggle(toggleRect, b[7]);
            toggleRect.x += position.height;
            b[6] = EditorGUI.Toggle(toggleRect, b[6]);
            toggleRect.x += position.height;
            b[5] = EditorGUI.Toggle(toggleRect, b[5]);
            toggleRect.x += position.height;
            b[4] = EditorGUI.Toggle(toggleRect, b[4]);
            toggleRect.x += position.height;
            b[3] = EditorGUI.Toggle(toggleRect, b[3]);
            toggleRect.x += position.height;
            b[2] = EditorGUI.Toggle(toggleRect, b[2]);
            toggleRect.x += position.height;
            b[1] = EditorGUI.Toggle(toggleRect, b[1]);
            toggleRect.x += position.height;
            b[0] = EditorGUI.Toggle(toggleRect, b[0]);
            toggleRect.x += position.height;
            if (EditorGUI.EndChangeCheck())
            {
                val = 0;
            ...
            EditorGUI.BeginChangeCheck();
            var numRect = new Rect(toggleRect.x, position.y, position.width - toggleRect.x + position.height + 2,
                position.height);
            ...
            MaterialProperty decalSubParam, MaterialProperty isDecal)
        {
            ...
            if (tex.textureValue != null &&
                AssetDatabase.GetAssetPath(tex.textureValue).EndsWith(".gif", StringComparison.OrdinalIgnoreCase) &&
                Button(GetLoc("sConvertGif")))
            {
                int frameCount, loopXY, duration;
                float xScale, yScale;
                var savePath = lilTextureUtils.ConvertGifToAtlas(tex.textureValue, out frameCount, out loopXY,
                    out duration, out xScale, out yScale);
                ...
                decalSubParam.vectorValue = new Vector4(xScale, yScale, decalSubParam.vectorValue.z,
                    decalSubParam.vectorValue.w);

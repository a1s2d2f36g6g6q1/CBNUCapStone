...
#pragma warning disable 0618
            DrawCustomProperties(m_MaterialEditor, material, boxOuter, boxInnerHalf, boxInner, customBox,
                customToggleFont, GUI.skin.button);
#pragma warning restore 0618
        ...
        public class lilMaterialEditor : EditorWindow
        {
            ...
            private void OnGUI()
            {
                var materials = Selection.GetFiltered<Material>(SelectionMode.DeepAssets).Where(m => m.shader != null)
                    .Where(m => m.shader.name.Contains("lilToon")).ToArray();
                ...
                EditorGUILayout.LabelField("Selected Materials",
                    string.Join(", ", materials.Select(m => m.name).ToArray()), EditorStyles.boldLabel);
            ...
            {
                var window =
                    (lilMaterialEditor)GetWindow(typeof(lilMaterialEditor), false, "[Beta] lilToon Multi-Editor");
                window.Show();
            }
        ...
        public void DrawAllGUI(MaterialEditor materialEditor, MaterialProperty[] props, Material material)
        {
            ...
            if (PlayerSettings.stripUnusedMeshComponents &&
                lilEditorGUI.AutoFixHelpBox(GetLoc("sWarnOptimiseMeshData")))
            ...
            switch (edSet.editorMode)
            {
                case EditorMode.Simple:
                    DrawSimpleGUI(material);
                    break;
                case EditorMode.Advanced:
                    DrawAdvancedGUI(material);
                    break;
            ...
            if (EditorGUI.EndChangeCheck())
            {
                ...
                if (mainColor != null && baseColor != null && !mainColor.hasMixedValue)
                    baseColor.colorValue = mainColor.colorValue;
                if (mainTex != null && baseMap != null && !mainTex.hasMixedValue)
                    baseMap.textureValue = mainTex.textureValue;
                if (mainTex != null && baseColorMap != null && !mainTex.hasMixedValue)
                    baseColorMap.textureValue = mainTex.textureValue;

                if (lilShaderAPI.IsTextureLimitedAPI())
                {
                    var shaderSettingString = lilToonSetting.BuildShaderSettingString(shaderSetting, true);
                    lilToonSetting.CheckTextures(ref shaderSetting, material);
                    var newShaderSettingString = lilToonSetting.BuildShaderSettingString(shaderSetting, true);
                    if (shaderSettingString != newShaderSettingString) lilToonSetting.ApplyShaderSetting(shaderSetting);
                }
            }
        }
        ...
        {
            return lilLanguageManager.BuildParams(labels);
        }

        public static void LoadCustomLanguage(string langFileGUID)
        {
            lilLanguageManager.LoadCustomLanguage(langFileGUID);
        }

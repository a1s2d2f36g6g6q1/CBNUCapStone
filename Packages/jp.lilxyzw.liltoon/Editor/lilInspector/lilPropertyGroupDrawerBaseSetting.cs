...
namespace lilToon
{
    ...
    {
        private void DrawRenderingModeSettings(Material material, string sTransparentMode, string[] sRenderingModeList,
            string[] sRenderingModeListLite)
        {
            if (isMultiVariants)
            {
                GUI.enabled = false;
            ...
            else
            {
                if (isShowRenderMode && !isMulti)
                {
                    ...
                    if (isLite)
                        renderingMode = (RenderingMode)lilEditorGUI.Popup(GetLoc("sRenderingMode"),
                            (int)renderingModeBuf, sRenderingModeListLite);
                    else
                        renderingMode = (RenderingMode)lilEditorGUI.Popup(GetLoc("sRenderingMode"),
                            (int)renderingModeBuf, sRenderingModeList);
                    if (renderingModeBuf != renderingMode)
                    {
                        SetupMaterialWithRenderingMode(renderingMode, transparentModeBuf);
                        if (renderingMode == RenderingMode.Cutout || renderingMode == RenderingMode.FurCutout)
                            cutoff.floatValue = 0.5f;
                        if (renderingMode == RenderingMode.Transparent || renderingMode == RenderingMode.Fur ||
                            renderingMode == RenderingMode.FurTwoPass) cutoff.floatValue = 0.001f;
                ...
                {
                    var transparentModeMatBuf = transparentModeMat.floatValue;
                    ...
                    if (transparentModeMatBuf != transparentModeMat.floatValue)
                    {
                        ...
                        if (transparentModeMat.floatValue == 1.0f || transparentModeMat.floatValue == 5.0f)
                            cutoff.floatValue = 0.5f;
                        if (transparentModeMat.floatValue == 2.0f || transparentModeMat.floatValue == 4.0f)
                ...
                {
                    var transparentMode = (TransparentMode)lilEditorGUI.Popup(GetLoc("sTransparentMode"),
                        (int)transparentModeBuf, sTransparentModeList);
                    if (transparentModeBuf != transparentMode)
                        SetupMaterialWithRenderingMode(renderingModeBuf, transparentMode);
                    if (transparentModeBuf == TransparentMode.OnePass && vertexLightStrength.floatValue != 1.0f &&
                        lilRenderPipelineReader.GetRP() == lilRenderPipeline.BRP &&
                        lilEditorGUI.AutoFixHelpBox(GetLoc("sHelpOnePassVertexLight")))
                        vertexLightStrength.floatValue = 1.0f;
                }

                if (renderingModeBuf == RenderingMode.Fur || renderingModeBuf == RenderingMode.FurCutout ||
                ...
                {
                    if (renderingModeBuf == RenderingMode.Refraction ||
                        renderingModeBuf == RenderingMode.RefractionBlur ||
                        renderingModeBuf == RenderingMode.Gem)
        ...
        private void DrawBaseSettings(Material material, string sTransparentMode, string[] sRenderingModeList,
            string[] sRenderingModeListLite, string[] sTransparentModeList)
        {
            ...
            if (edSet.isShowBase)
            {
                ...
                if (!isGem && !isFakeShadow)
                {
                    ...
                    if (cull.floatValue <= 1.0f ||
                        (transparentModeBuf == TransparentMode.TwoPass && preCull.floatValue <= 1.0f))
                    {
                ...
                if ((!isFakeShadow && renderingModeBuf == RenderingMode.Cutout) ||
                    (isMulti && transparentModeMat.floatValue == 1.0f))
                {
                    ...
                    if (lilEditorGUI.CheckPropertyToDraw(ditherTex, ditherMaxValue) && useDither.floatValue == 1.0f)
                    {
                        ...
                        if (EditorGUI.EndChangeCheck() && ditherTex.textureValue != null)
                            ditherMaxValue.floatValue =
                                Mathf.Clamp(ditherTex.textureValue.width * ditherTex.textureValue.height - 1, 0, 255);
                        LocalizedProperty(ditherMaxValue);
                        EditorGUILayout.BeginHorizontal();
                        GUILayout.Space(16);
                        if (GUILayout.Button("x2"))
                        {
                            ditherTex.textureValue =
                                AssetDatabase.LoadAssetAtPath<Texture2D>(lilDirectoryManager.GetMainFolderPath() +
                                                                         "/Texture/lil_bayer_2x2.png");
                            ditherMaxValue.floatValue = 3;
                        }
                        ...
                        if (GUILayout.Button("x4"))
                        {
                            ditherTex.textureValue =
                                AssetDatabase.LoadAssetAtPath<Texture2D>(lilDirectoryManager.GetMainFolderPath() +
                                                                         "/Texture/lil_bayer_4x4.png");
                            ditherMaxValue.floatValue = 15;
                        }
                        ...
                        if (GUILayout.Button("x8"))
                        {
                            ditherTex.textureValue =
                                AssetDatabase.LoadAssetAtPath<Texture2D>(lilDirectoryManager.GetMainFolderPath() +
                                                                         "/Texture/lil_bayer_8x8.png");
                            ditherMaxValue.floatValue = 63;
                        }

                        if (GUILayout.Button("x16"))
                        {
                            ditherTex.textureValue =
                                AssetDatabase.LoadAssetAtPath<Texture2D>(lilDirectoryManager.GetMainFolderPath() +
                                                                         "/Texture/lil_bayer_16x16.png");
                            ditherMaxValue.floatValue = 255;
                        }
                ...
                if (transparentModeBuf == TransparentMode.TwoPass)
                {
                    ...
                    preBlendMode =
                        lilEditorGUI.Popup(
                            Event.current.alt ? preSrcBlend.name + ", " + preDstBlend.name : GetLoc("sBlendMode"),
                            preBlendMode, sBlendModeList);
                    ...
                    edSet.isShowPrePreset =
                        lilEditorGUI.DrawSimpleFoldout(GetLoc("sPresets"), edSet.isShowPrePreset, isCustomEditor);
                    if (edSet.isShowPrePreset)
                    {
                        ...
                        if (lilEditorGUI.Button(GetLoc("sTransparentPresetsPreWriteDepth")))
                        {
                            ...
                            mainColor.colorValue = new Color(mainColor.colorValue.r, mainColor.colorValue.g,
                                mainColor.colorValue.b, 1.0f);
                ...
                EditorGUILayout.BeginVertical(customBox);
                var stencilMode = -1;
                if (stencilComp.floatValue == (float)CompareFunction.Always &&
                    stencilPass.floatValue == (float)StencilOp.Keep) stencilMode = 0; // Normal
                if (stencilComp.floatValue == (float)CompareFunction.Always &&
                    stencilPass.floatValue == (float)StencilOp.Replace) stencilMode = 1; // Writer
                if (stencilComp.floatValue == (float)CompareFunction.NotEqual &&
                    stencilPass.floatValue == (float)StencilOp.Keep) stencilMode = 2; // Reader
                if (stencilComp.floatValue == (float)CompareFunction.Equal &&
                    stencilPass.floatValue == (float)StencilOp.Keep) stencilMode = 3; // Reader (Invert)
                if (transparentModeBuf == TransparentMode.TwoPass &&
                ...
                var outlineStencilMode = -1;

                EditorGUI.BeginChangeCheck();
                if (transparentModeBuf == TransparentMode.TwoPass)
                    stencilMode = lilEditorGUI.Popup("Mode", stencilMode, GetLoc("sStencilModeNormal"),
                        GetLoc("sStencilModeWriter"), GetLoc("sStencilModeReader"), GetLoc("sStencilModeReaderInvert"),
                        GetLoc("sStencilModeReaderFade"));
                else
                    stencilMode = lilEditorGUI.Popup("Mode", stencilMode, GetLoc("sStencilModeNormal"),
                        GetLoc("sStencilModeWriter"), GetLoc("sStencilModeReader"), GetLoc("sStencilModeReaderInvert"));
                if (isOutl)
                {
                    if (outlineStencilComp.floatValue == (float)CompareFunction.Always &&
                        outlineStencilPass.floatValue == (float)StencilOp.Keep) outlineStencilMode = 0; // Normal
                    if (outlineStencilComp.floatValue == (float)CompareFunction.Always &&
                        outlineStencilPass.floatValue == (float)StencilOp.Replace) outlineStencilMode = 1; // Writer
                    if (outlineStencilComp.floatValue == (float)CompareFunction.NotEqual &&
                        outlineStencilPass.floatValue == (float)StencilOp.Keep) outlineStencilMode = 2; // Reader
                    if (outlineStencilComp.floatValue == (float)CompareFunction.Equal &&
                        outlineStencilPass.floatValue == (float)StencilOp.Keep)
                        outlineStencilMode = 3; // Reader (Invert)
                    outlineStencilMode = lilEditorGUI.Popup("Mode (" + GetLoc("sOutline") + ")", outlineStencilMode,
                        GetLoc("sStencilModeNormal"), GetLoc("sStencilModeWriter"), GetLoc("sStencilModeReader"),
                        GetLoc("sStencilModeReaderInvert"));
                }

                if (EditorGUI.EndChangeCheck())
                {
                    SetupMaterialWithRenderingMode(renderingModeBuf, transparentModeBuf);
                    var shaderRenderQueue = isMulti ? material.renderQueue : material.shader.renderQueue;
                    switch (stencilMode)
                    {
                        case 0:
                    ...
                    if (transparentModeBuf == TransparentMode.TwoPass)
                    {
                        ztest.floatValue = stencilMode == 4
                            ? (float)CompareFunction.Less
                            : (float)CompareFunction.LessEqual;
                        preStencilRef.floatValue = stencilRef.floatValue;
                        preStencilComp.floatValue =
                            stencilMode == 4 ? (float)CompareFunction.Equal : stencilComp.floatValue;
        ...
        private void DrawLightingSettings()
        {
            ...
            if (edSet.isShowLightingSettings)
            {
                ...
                var button = lilEditorGUI.Buttons(GetLoc("sLightingPreset"), GetLoc("sLightingPresetDefault"),
                    GetLoc("sLightingPresetSemiMonochrome"));
        ...
        private void BlendOpFASetting()
        {
            ...
            var selecting = blendOpFA.floatValue == 0 ? 0 : blendOpFA.floatValue == 4 ? 1 : 2;
            EditorGUI.BeginChangeCheck();
            EditorGUI.showMixedValue = blendOpFA.hasMixedValue;
            selecting = lilEditorGUI.Popup(Event.current.alt ? blendOpFA.name : GetLoc("sLightBlending"), selecting,
                GetLoc("sBlendingAdd"), GetLoc("sBlendingMax"));
            EditorGUI.showMixedValue = false;

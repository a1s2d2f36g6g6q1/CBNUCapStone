...
namespace lilToon
{
    public partial class lilToonInspector
    {
        ...
        private static void DrawWebPages()
        {
            ...
            var versionLabel = "lilToon " + lilConstants.currentVersionName;
            if (latestVersion != null && latestVersion.latest_vertion_name != null &&
                latestVersion.latest_vertion_value > lilConstants.currentVersionValue)
            {
                versionLabel = "[Update] lilToon " + lilConstants.currentVersionName + " -> " +
                               latestVersion.latest_vertion_name;
                labelStyle.normal.textColor = Color.red;
            }

            edSet.isShowWebPages =
                lilEditorGUI.DrawSimpleFoldout(versionLabel, edSet.isShowWebPages, labelStyle, isCustomEditor);
        ...
        {
            edSet.isShowHelpPages =
                lilEditorGUI.DrawSimpleFoldout(GetLoc("sHelp"), edSet.isShowHelpPages, isCustomEditor);
            if (edSet.isShowHelpPages)
            {
                EditorGUI.indentLevel++;
                lilEditorGUI.DrawWebButton(GetLoc("sCommonProblems"),
                    GetLoc("sReadmeURL") + GetLoc("sReadmeAnchorProblem"));
                EditorGUI.indentLevel--;
            }
        }
        ...
        {
            if (!isMultiVariants && material.shader.name.Contains("Overlay") &&
                lilEditorGUI.AutoFixHelpBox(GetLoc("sHelpSelectOverlay"))) material.shader = lts;
        }
        ...
        private static void SelectEditorMode()
        {
            ...
            {
                GetLoc("sEditorModeSimple"), GetLoc("sEditorModeAdvanced"), GetLoc("sEditorModePreset"),
        ...
        private void DrawMenuButton(string helpAnchor, PropertyBlock propertyBlock)
        {
            ...
            if (GUI.Button(position, EditorGUIUtility.IconContent("_Popup"), middleButton))
            {
                ...
                menu.AddItem(new GUIContent(GetLoc("sPaste")), false, PasteProperties,
                    new PropertyBlockData { propertyBlock = propertyBlock, shouldCopyTex = false });
                menu.AddItem(new GUIContent(GetLoc("sPasteWithTexture")), false, PasteProperties,
                    new PropertyBlockData { propertyBlock = propertyBlock, shouldCopyTex = true });
 ...
 m_MaterialEditor.targets.Where(obj => obj is Material).Any(obj => tag != ((Material)obj).GetTag("VRCFallback", false));
                    bool shouldSetTag =
 EditorGUI.ToggleLeft(EditorGUILayout.GetControlRect(), "Custom Safety Fallback", !string.IsNullOrEmpty(tag), customToggleFont);
                    if(shouldSetTag)
                    {
                        EditorGUILayout.BeginVertical(boxInnerHalf);
                        string[] sFallbackShaderTypes =
 {"Unlit", "Standard", "VertexLit", "Toon", "Particle", "Sprite", "Matcap", "MobileToon", "Hidden"};
                        ...
                        fallbackShaderType =
 lilEditorGUI.Popup("Shader Type", fallbackShaderType, sFallbackShaderTypes);
                        fallbackRenderType =
 lilEditorGUI.Popup("Rendering Mode", fallbackRenderType, sFallbackRenderTypes);
        ...
        private void PasteProperties(PropertyBlock propertyBlock, bool shouldCopyTex)
        {
                     ...
                     ))
            {
                ...
                if (propType == MaterialProperty.PropType.Texture)
                    p.textureValue = copiedProperties[p.name].textureValue;
            }
        }
        ...
        private void ResetProperties(PropertyBlock propertyBlock)
        {
                     ...
                     ))
            {
                ...
                var propID = shader.FindPropertyIndex(p.name);
                if (propID == -1) continue;
                var propType = p.type;
                if (propType == MaterialProperty.PropType.Color)
                    p.colorValue = shader.GetPropertyDefaultVectorValue(propID);
                if (propType == MaterialProperty.PropType.Vector)
                    p.vectorValue = shader.GetPropertyDefaultVectorValue(propID);
                if (propType == MaterialProperty.PropType.Float)
                    p.floatValue = shader.GetPropertyDefaultFloatValue(propID);
                if (propType == MaterialProperty.PropType.Range)
                    p.floatValue = shader.GetPropertyDefaultFloatValue(propID);
        ...
        private bool ShouldDrawBlock(PropertyBlock propertyBlock)
        {
            ...
            if (propertyBlock == PropertyBlock.Rendering &&
                lilEditorGUI.CheckPropertyToDraw("Render Queue", "Enable GPU Instancing")) return true;
        ...
        private void ApplyLightingPreset(LightingPreset lightingPreset)
        {
            switch (lightingPreset)
            {
                case LightingPreset.Default:
                    ...
                    if (vertexLightStrength.p != null)
                        vertexLightStrength.floatValue = shaderSetting.defaultVertexLightStrength;
                    ...
                    if (lilDirectionalLightStrength.p != null)
                        lilDirectionalLightStrength.floatValue = shaderSetting.defaultlilDirectionalLightStrength;
                    break;

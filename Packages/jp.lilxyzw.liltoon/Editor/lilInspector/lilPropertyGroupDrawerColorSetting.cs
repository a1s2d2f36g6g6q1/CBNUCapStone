...
namespace lilToon
{
    public partial class lilToonInspector
    {
        ...
        {
            edSet.isShowMainTone =
                lilEditorGUI.DrawSimpleFoldout(GetLoc("sColorAdjust"), edSet.isShowMainTone, isCustomEditor);
            if (edSet.isShowMainTone)
            {
                ...
                if (mainGradationStrength.floatValue != 0 && (lilEditorGUI.CheckPropertyToDraw(gradationMapContent) ||
                                                              lilEditorGUI.CheckPropertyToDraw(mainGradationTex)))
                {
        ...
        private void DrawAlphaMaskSettings(Material material)
        {
            ...
            if ((renderingModeBuf == RenderingMode.Opaque && !isMulti) ||
            ...
            else
            {
                ...
                if (alphaMaskMode.floatValue != 0)
                {
                    ...
                    var invertAlphaMask = alphaMaskScale.floatValue < 0;
                    var transparency = alphaMaskValue.floatValue - (invertAlphaMask ? 1.0f : 0.0f);
                    ...
                    transparency =
                        lilEditorGUI.Slider(
                            Event.current.alt ? alphaMaskScale.name + ", " + alphaMaskValue.name : "Transparency",
                            transparency, -1.0f, 1.0f);
                    ...
                    edSet.isAlphaMaskModeAdvanced =
                        EditorGUILayout.Toggle("Show advanced editor", edSet.isAlphaMaskModeAdvanced);
        ...
        private void DrawShadowSettings()
        {
            ...
            if (edSet.isShowShadow)
            {
                ...
                if (useShadow.floatValue == 1 && !isLite)
                {
                    ...
                    lilEditorGUI.DrawLine();
                    edSet.isShowShadowAO = lilEditorGUI.DrawSimpleFoldout(m_MaterialEditor, shadowAOMapContent,
                        shadowBorderMask, edSet.isShowShadowAO, isCustomEditor);
                    if (edSet.isShowShadowAO)
                    {
                        EditorGUI.indentLevel += 1;
                        LocalizedProperty(shadowBorderMaskLOD);
                        LocalizedProperty(shadowPostAO);
                        var min1 = lilEditorGUI.GetRemapMinValue(shadowAOShift.vectorValue.x,
                            shadowAOShift.vectorValue.y);
                        var max1 = lilEditorGUI.GetRemapMaxValue(shadowAOShift.vectorValue.x,
                            shadowAOShift.vectorValue.y);
                        var min2 = lilEditorGUI.GetRemapMinValue(shadowAOShift.vectorValue.z,
                            shadowAOShift.vectorValue.w);
                        var max2 = lilEditorGUI.GetRemapMaxValue(shadowAOShift.vectorValue.z,
                            shadowAOShift.vectorValue.w);
                        var min3 = lilEditorGUI.GetRemapMinValue(shadowAOShift2.vectorValue.x,
                            shadowAOShift2.vectorValue.y);
                        var max3 = lilEditorGUI.GetRemapMaxValue(shadowAOShift2.vectorValue.x,
                            shadowAOShift2.vectorValue.y);
                        EditorGUI.BeginChangeCheck();
                        EditorGUI.showMixedValue = alphaMaskScale.hasMixedValue || alphaMaskValue.hasMixedValue;
                        min1 = lilEditorGUI.Slider(Event.current.alt ? shadowAOShift.name : "1st Min", min1, -0.01f,
                            1.01f);
                        max1 = lilEditorGUI.Slider(Event.current.alt ? shadowAOShift.name : "1st Max", max1, -0.01f,
                            1.01f);
                        min2 = lilEditorGUI.Slider(Event.current.alt ? shadowAOShift.name : "2nd Min", min2, -0.01f,
                            1.01f);
                        max2 = lilEditorGUI.Slider(Event.current.alt ? shadowAOShift.name : "2nd Max", max2, -0.01f,
                            1.01f);
                        min3 = lilEditorGUI.Slider(Event.current.alt ? shadowAOShift2.name : "3rd Min", min3, -0.01f,
                            1.01f);
                        max3 = lilEditorGUI.Slider(Event.current.alt ? shadowAOShift2.name : "3rd Max", max3, -0.01f,
                            1.01f);
                        EditorGUI.showMixedValue = false;

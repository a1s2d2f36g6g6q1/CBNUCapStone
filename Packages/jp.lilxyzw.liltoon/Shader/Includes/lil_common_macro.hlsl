...
float3 lilTransformDirOStoWS(float3 directionOS, bool doNormalize)
{
    ...
    return mul((float3x3)LIL_MATRIX_M, directionOS);
}
...
float3 lilTransformDirWStoOS(float3 directionWS, bool doNormalize)
{
    ...
    return mul((float3x3)LIL_MATRIX_I_M, directionWS);
}
...
float3 lilTransformDirWStoVSCenter(float3 directionWS, bool doNormalize)
{
    ...
    return mul((float3x3)LIL_MATRIX_V, directionWS);
    #endif
}
...
// Additional Light
void lilGetAdditionalLights(float3 positionWS, float4 positionCS, float strength, inout float3 lightColor,
...
Unity_GlossyEnvironmentData lilSetupGlossyEnvironmentData(float3 viewDirection, float3 normalDirection,
...
float3 lilGetEnvReflection(float3 viewDirection, float3 normalDirection, float perceptualRoughness, float3 positionWS)
{
    ...
    Unity_GlossyEnvironmentData glossIn = lilSetupGlossyEnvironmentData(viewDirection, normalDirection,
                                                                        perceptualRoughness);

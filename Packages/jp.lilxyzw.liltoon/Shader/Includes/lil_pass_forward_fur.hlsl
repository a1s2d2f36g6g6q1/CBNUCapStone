...
float4 frag(v2f input) : SV_Target
{
    ...
    fd.col.rgb += input.furLayer * pow((1 - abs(dot(normalize(fd.N), fd.V))), _FurRimFresnelPower) * lerp(
        1, lilGray(fd.invLighting), _FurRimAntiLight) * _FurRimColor.rgb * fd.lightColor;

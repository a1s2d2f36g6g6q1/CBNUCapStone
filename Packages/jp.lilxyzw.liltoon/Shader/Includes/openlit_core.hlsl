...
float3 ComputeCustomLightDirection(float4 lightDirectionOverride)
{
    float3 customDir = length(lightDirectionOverride.xyz) * normalize(
        mul((float3x3)OPENLIT_MATRIX_M, lightDirectionOverride.xyz));
...
//------------------------------------------------------------------------------------------------------------------------------
// Correct
void CorrectLights(inout OpenLitLightDatas lightDatas, float lightMinLimit, float lightMaxLimit,
                   float monochromeLighting, float asUnlit)
{
...
float3 ComputeAdditionalLights(float3 positionWS, float3 positionCS)
{
    ...
    float4 atten = saturate(
        saturate((25.0 - lengthSq * unity_4LightAtten0) * 0.111375) / (0.987725 + lengthSq * unity_4LightAtten0));

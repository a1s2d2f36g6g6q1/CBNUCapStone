...
lilTessellationFactors hullConst(InputPatch<appdataCopy, 3> input)
{
    ...
    // Rim
    float3 nv = float3(abs(dot(vertexNormalInput_0.normalWS,
                               normalize(lilViewDirection(lilToAbsolutePositionWS(vertexInput_0.positionWS))))),
                       abs(dot(vertexNormalInput_1.normalWS,
                               normalize(lilViewDirection(lilToAbsolutePositionWS(vertexInput_1.positionWS))))),
                       abs(dot(vertexNormalInput_2.normalWS,
                               normalize(lilViewDirection(lilToAbsolutePositionWS(vertexInput_2.positionWS))))));
...
#endif
{
    ...
    float3 pt[3];
    for (int i = 0; i < 3; i++)
        pt[i] = input[i].normalOS * (dot(input[i].positionOS.xyz, input[i].normalOS) - dot(
            output.positionOS.xyz, input[i].normalOS) - _TessShrink * 0.01);

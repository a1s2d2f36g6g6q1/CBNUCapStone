...
void lilCustomVertexWS(inout appdata input, inout float2 uvMain, inout lilVertexPositionInputs vertexInput,
...
v2g vert(appdata input)
{
    ...
    if (_UDIMDiscardMode == 0 && _UDIMDiscardCompile == 1 && LIL_CHECK_UDIMDISCARD(input))
    // Discard Vertices instead of just pixels
    {
    ...
    float3 bitangentOS = normalize(cross(input.normalOS, input.tangentOS.xyz)) * (input.tangentOS.w * length(
        input.normalOS));
    ...
    output.furVector = lilBlendNormal(output.furVector,
                                      lilUnpackNormalScale(
                                          LIL_SAMPLE_2D_LOD(_FurVectorTex, lil_sampler_linear_repeat, uvMain, 0),
                                          _FurVectorScale));
    #endif
    ...
    int idMaskIndices[8] = {
        _IDMaskIndex1, _IDMaskIndex2, _IDMaskIndex3, _IDMaskIndex4, _IDMaskIndex5, _IDMaskIndex6, _IDMaskIndex7,
        _IDMaskIndex8
    };
    ...
    case 0: idMaskArg = input.uv0.x;
        break;
        #endif
        #if defined(LIL_APP_TEXCOORD1)
    case 1: idMaskArg = input.uv1.x;
        break;
        #endif
        #if defined(LIL_APP_TEXCOORD2)
    case 2: idMaskArg = input.uv2.x;
        break;
        #endif
        #if defined(LIL_APP_TEXCOORD3)
    case 3: idMaskArg = input.uv3.x;
        break;
        #endif
        #if defined(LIL_APP_TEXCOORD4)
    case 4: idMaskArg = input.uv4.x;
        break;
        #endif
        #if defined(LIL_APP_TEXCOORD5)
    case 5: idMaskArg = input.uv5.x;
        break;
        #endif
        #if defined(LIL_APP_TEXCOORD6)
    case 6: idMaskArg = input.uv6.x;
        break;
        #endif
        #if defined(LIL_APP_TEXCOORD7)
    case 7: idMaskArg = input.uv7.x;
        break;
        #endif
    default: idMaskArg = input.vertexID;
        break;
    }
...
void geom(triangle v2g input[3], inout TriangleStream<v2f> outStream)
{
    ...
    else
    {
        ...
        uint3 n0 = (input[0].vertexID * 3 + input[1].vertexID * 1 + input[2].vertexID * 1) * uint3(
            1597334677U, 3812015801U, 2912667907U);
        uint3 n1 = (input[0].vertexID * 1 + input[1].vertexID * 3 + input[2].vertexID * 1) * uint3(
            1597334677U, 3812015801U, 2912667907U);
        uint3 n2 = (input[0].vertexID * 1 + input[1].vertexID * 1 + input[2].vertexID * 3) * uint3(
            1597334677U, 3812015801U, 2912667907U);
        ...
        furVectors[0] *= LIL_SAMPLE_2D_LOD(_FurLengthMask, lil_sampler_linear_repeat,
                                           input[0].uv0 * _MainTex_ST.xy + _MainTex_ST.zw, 0).r;
        furVectors[1] *= LIL_SAMPLE_2D_LOD(_FurLengthMask, lil_sampler_linear_repeat,
                                           input[1].uv0 * _MainTex_ST.xy + _MainTex_ST.zw, 0).r;
        furVectors[2] *= LIL_SAMPLE_2D_LOD(_FurLengthMask, lil_sampler_linear_repeat,
                                           input[2].uv0 * _MainTex_ST.xy + _MainTex_ST.zw, 0).r;
        #endif

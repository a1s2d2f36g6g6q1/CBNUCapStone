...
void lilAudioLinkFrag(inout lilFragData fd LIL_SAMP_IN_FUNC(samp))
{
    if (_UseAudioLink)
    {
        ...
        if (_AudioLinkUVMode == 1) audioLinkUV.x = _AudioLinkUVParams.r - fd.nv * _AudioLinkUVParams.r +
            _AudioLinkUVParams.g;
        if (_AudioLinkUVMode == 2) audioLinkUV.x = lilRotateUV(fd.uv0, _AudioLinkUVParams.b).x * _AudioLinkUVParams.r +
            _AudioLinkUVParams.g;
        if (_AudioLinkUVMode == 5) audioLinkUV.x = distance(fd.positionOS, _AudioLinkStart.xyz) * _AudioLinkUVParams.r +
            _AudioLinkUVParams.g;
        ...
        if (_AudioLinkUVMode == 3 || _AudioLinkUVMode == 4)
        {
            ...
            audioLinkUV = _AudioLinkUVMode == 3
                              ? audioLinkMask.rg
                              : float2(frac(audioLinkMask.g * 2.0), 4.5 / 4.0 + floor(audioLinkMask.g * 2.0) / 4.0);
        }
        #endif
        ...
        if (_AudioLinkUVMode == 4)
        {
            ...
            float defaultVal = sin(LIL_TIME * _AudioLinkDefaultValue.z - audioLinkMask.g * _AudioLinkDefaultValue.y) *
                _AudioLinkDefaultValue.x * _AudioLinkUVParams.x + _AudioLinkDefaultValue.x * _AudioLinkUVParams.x;
            fd.audioLinkValue = _AudioLinkUVParams.w < 1.0
                                    ? abs(defaultVal - defaultY) < _AudioLinkUVParams.w
                                    : defaultVal > defaultY;
        }
        else
        {
            fd.audioLinkValue = saturate(
                _AudioLinkDefaultValue.x - saturate(
                    frac(LIL_TIME * _AudioLinkDefaultValue.z - audioLinkUV.x) + _AudioLinkDefaultValue.w) *
                _AudioLinkDefaultValue.y * _AudioLinkDefaultValue.x);
        }
        ...
        {
            audioLinkUV.x += frac(-LIL_TIME * _AudioLinkLocalMapParams.r / 60 * _AudioLinkLocalMapParams.g) +
                _AudioLinkLocalMapParams.b;
        ...
        // Global
            if (lilCheckAudioLink())
            {
                ...
                if (_AudioLinkUVMode == 4)
                {
                    float audioVal = audioTexture.b * _AudioLinkUVParams.x * lerp(
                        _AudioLinkUVParams.y, _AudioLinkUVParams.z, audioLinkMask.g);
                    fd.audioLinkValue = _AudioLinkUVParams.w < 1.0
                                            ? abs(audioVal - audioLinkMask.r) < _AudioLinkUVParams.w
                                            : audioVal > audioLinkMask.r;
                }
                else
                {
...
void lilGetMain2nd(inout lilFragData fd, inout float4 color2nd, inout float main2ndDissolveAlpha LIL_SAMP_IN_FUNC(samp))
{
    ...
    if (_UseMain2ndTex)
    {
        ...
        #endif
        color2nd.a = lerp(
            color2nd.a, color2nd.a * saturate(
                (fd.depth - _Main2ndDistanceFade.x) / (_Main2ndDistanceFade.y - _Main2ndDistanceFade.x)),
...
void lilGetMain3rd(inout lilFragData fd, inout float4 color3rd, inout float main3rdDissolveAlpha LIL_SAMP_IN_FUNC(samp))
{
    ...
    if (_UseMain3rdTex)
    {
        ...
        #endif
        color3rd.a = lerp(
            color3rd.a, color3rd.a * saturate(
                (fd.depth - _Main3rdDistanceFade.x) / (_Main3rdDistanceFade.y - _Main3rdDistanceFade.x)),
...
void lilGetShading(inout lilFragData fd LIL_SAMP_IN_FUNC(samp))
{
    if (_UseShadow)
    {
        ...
        #else
        shadowStrengthMask = LIL_SAMPLE_2D_GRAD(_ShadowStrengthMask, lil_sampler_linear_repeat, fd.uvMain,
                                                max(fd.ddxMain, _ShadowStrengthMaskLOD),
                                                max(fd.ddyMain, _ShadowStrengthMaskLOD));
        #endif
        #endif
        ...
        #else
        float4 shadowBlurMask = LIL_SAMPLE_2D_GRAD(_ShadowBlurMask, lil_sampler_linear_repeat, fd.uvMain,
                                                   max(fd.ddxMain, _ShadowBlurMaskLOD),
                                                   max(fd.ddyMain, _ShadowBlurMaskLOD));
        #endif
        ...
        #else
        float4 shadowBorderMask = LIL_SAMPLE_2D_GRAD(_ShadowBorderMask, lil_sampler_linear_repeat, fd.uvMain,
                                                     max(fd.ddxMain, _ShadowBorderMaskLOD),
                                                     max(fd.ddyMain, _ShadowBorderMaskLOD));
        #endif
...
void lilGetMatCap(inout lilFragData fd LIL_SAMP_IN_FUNC(samp))
{
    if (_UseMatCap)
    {
        ...
        // UV
        float2 matUV = lilCalcMatCapUV(fd.uv1, normalize(N), fd.V, fd.headV, _MatCapTex_ST, _MatCapBlendUV1.xy,
                                       _MatCapZRotCancel, _MatCapPerspective, _MatCapVRParallaxStrength);
        ...
        fd.col.rgb = lilBlendColor(fd.col.rgb, matCapColor.rgb, _MatCapBlend * matCapColor.a * matCapMask,
                                   _MatCapBlendMode);
    }
}
#elif defined(LIL_LITE)
...
void lilGetMatCap2nd(inout lilFragData fd LIL_SAMP_IN_FUNC(samp))
{
    if (_UseMatCap2nd)
    {
        ...
        // UV
        float2 mat2ndUV = lilCalcMatCapUV(fd.uv1, N, fd.V, fd.headV, _MatCap2ndTex_ST, _MatCap2ndBlendUV1.xy,
                                          _MatCap2ndZRotCancel, _MatCap2ndPerspective, _MatCap2ndVRParallaxStrength);
        ...
        fd.col.rgb = lilBlendColor(fd.col.rgb, matCap2ndColor.rgb, _MatCap2ndBlend * matCap2ndColor.a * matCapMask,
                                   _MatCap2ndBlendMode);
    }
}
#endif
...
void lilGetRim(inout lilFragData fd LIL_SAMP_IN_FUNC(samp))
{
    if (_UseRim)
    {
        ...
        fd.col.rgb = lilBlendColor(fd.col.rgb, rimIndirColor.rgb * rimLightMul, rimIndir * rimIndirColor.a,
                                   _RimBlendMode);
        #else
                // Color
                float4 rimColor = _RimColor;
...
void lilGlitter(inout lilFragData fd LIL_SAMP_IN_FUNC(samp))
{
    if (_UseGlitter)
    {
        ...
        glitterColor.rgb *= lilCalcGlitter(glitterPos, N, glitterViewDirection, glitterCameraDirection, fd.L,
                                           _GlitterParams1, _GlitterParams2, _GlitterPostContrast, _GlitterSensitivity,
                                           _GlitterScaleRandomize, _GlitterAngleRandomize, _GlitterApplyShape,
                                           _GlitterShapeTex, _GlitterShapeTex_ST, _GlitterAtras);
        #else
...
void lilEmission2nd(inout lilFragData fd LIL_SAMP_IN_FUNC(samp))
{
    if (_UseEmission2nd)
    {
        ...
        #endif
        emission2ndColor.rgb = lerp(emission2ndColor.rgb, emission2ndColor.rgb * fd.invLighting,

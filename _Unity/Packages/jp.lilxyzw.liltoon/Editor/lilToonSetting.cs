...
{
    private static readonly HashSet<string> logs = new();
    ...
    public bool LIL_OPTIMIZE_USE_FORWARDADD_SHADOW;
    public bool LIL_OPTIMIZE_USE_VERTEXLIGHT = true;
    public bool LIL_OPTIMIZE_USE_LIGHTMAP;
    public bool LIL_OPTIMIZE_DEFFERED;

    public bool isLocked;
    public bool isDebugOptimize;
    public bool isOptimizeInTestBuild;
    public bool isOptimizeInNDMF;
    public bool isMigrateInStartUp = true;

    public float defaultAsUnlit;
    public float defaultVertexLightStrength;
    ...
    public float defaultMonochromeLighting;
    ...
    {
        var path = lilDirectoryManager.GetSettingLockPath();
    ...
    {
        var shaderSettingPath = lilDirectoryManager.GetShaderSettingPath();
    ...
    {
        SaveShaderSetting(shaderSetting);
        var shaderSettingString = BuildShaderSettingString(shaderSetting, true);

        var shaderFolderPath = lilDirectoryManager.GetShaderFolderPath();
        var baseShaderFolderPath = lilDirectoryManager.GetBaseShaderFolderPath();
        foreach (var baseShaderPath in lilDirectoryManager.FindAssetsPath("", new[] { baseShaderFolderPath })
                     .Where(p => p.Contains(".lilinternal")))
        {
            var shaderPath = shaderFolderPath + Path.AltDirectorySeparatorChar +
                             Path.GetFileNameWithoutExtension(baseShaderPath) + ".shader";
            File.WriteAllText(shaderPath, lilShaderContainer.UnpackContainer(baseShaderPath));
        }

        foreach (var folder in lilDirectoryManager.FindAssetsPath("t:shader").Where(p => p.Contains(".lilcontainer"))
                     .Select(p => Path.GetDirectoryName(p))
                     .Distinct()) AssetDatabase.ImportAsset(folder, ImportAssetOptions.ImportRecursive);
        AssetDatabase.SaveAssets();
        AssetDatabase.Refresh();

        if (!string.IsNullOrEmpty(reportTitle)) Debug.Log(reportTitle + Environment.NewLine + shaderSettingString);
    }
        ...
        bool doOptimize = false)
    {
        ...
        foreach (var shader in shaders)
        {
            ...
            var baseShaderPath = baseShaderFolderPath + Path.AltDirectorySeparatorChar +
                                 Path.GetFileNameWithoutExtension(shaderPath) + ".lilinternal";
    ...
    public static string BuildShaderSettingString(lilToonSetting shaderSetting, bool isFile)
    {
        ...
        if (shaderSetting.LIL_FEATURE_EMISSION_1ST || shaderSetting.LIL_FEATURE_EMISSION_2ND)
        {
            ...
            if (shaderSetting.LIL_FEATURE_ANIMATE_EMISSION_MASK_UV)
                sb.AppendLine("#define LIL_FEATURE_ANIMATE_EMISSION_MASK_UV");
        ...
        if (shaderSetting.LIL_FEATURE_OUTLINE_TONE_CORRECTION)
            sb.AppendLine("#define LIL_FEATURE_OUTLINE_TONE_CORRECTION");
        if (shaderSetting.LIL_FEATURE_OUTLINE_RECEIVE_SHADOW)
            sb.AppendLine("#define LIL_FEATURE_OUTLINE_RECEIVE_SHADOW");
        ...
        if (shaderSetting.LIL_FEATURE_AnisotropyShiftNoiseMask)
            sb.AppendLine("#define LIL_FEATURE_AnisotropyShiftNoiseMask");
        ...
        if (shaderSetting.LIL_OPTIMIZE_USE_FORWARDADD_SHADOW)
            sb.AppendLine("#define LIL_OPTIMIZE_USE_FORWARDADD_SHADOW");
    ...
    public static string BuildShaderSettingString(bool isFile, ref bool useBaseShadow, ref bool useOutlineShadow)
    {
        lilToonSetting shaderSetting = null;
        InitializeShaderSetting(ref shaderSetting);
        useBaseShadow = (shaderSetting.LIL_FEATURE_SHADOW && shaderSetting.LIL_FEATURE_RECEIVE_SHADOW) ||
                        shaderSetting.LIL_FEATURE_BACKLIGHT;
        useOutlineShadow = shaderSetting.LIL_FEATURE_OUTLINE_RECEIVE_SHADOW;
        var shaderSettingString = BuildShaderSettingString(shaderSetting, isFile);
    ...
    internal static void WalkAllSceneReferencedAssets(Action<Object> callback, bool loadBuildScene)
    {
        ...
        if (loadBuildScene)
        {
            ...
            {
                if (i >= 0) EditorSceneManager.OpenScene(SceneUtility.GetScenePathByBuildIndex(i));
    ...
    private static void WalkAllSceneReferencedAssets(Action<Object> callback, Queue<Object> toVisit,
    ...
    internal static void GetOptimizedSetting(Material[] materials, AnimationClip[] clips, out string usedShaders,
        out string optimizedHLSL, out string shaderSettingText)
    {
        ...
        // Get materials
        foreach (var material in materials) SetupShaderSettingFromMaterial(material, ref shaderSetting);

        // Get animations
        foreach (var clip in clips) SetupShaderSettingFromAnimationClip(clip, ref shaderSetting, true);
    ...
    internal static void SetShaderSettingBeforeBuild(Material[] materials, AnimationClip[] clips)
    {
...
#if UNITY_2022_1_OR_NEWER || (UNITY_2023_1_OR_NEWER && !UNITY_2023_2_OR_NEWER)
        if (WorkaroundForUsePassBug())
        {
            Debug.Log("[lilToon] Skip Optimization");
            return;
        }
#endif
        try
        {
            ...
            foreach (var m in materials) GetMaterialParents(materialParents, m);
            ...
            lilEditorParameters.instance.modifiedShaders =
                string.Join(",", shaders.Select(s => s.name).Distinct().ToArray());
    ...
    internal static void SetShaderSettingAfterBuild()
    {
        try
        {
            ...
            var shaders = lilEditorParameters.instance.modifiedShaders.Split(',').Select(n => Shader.Find(n))
    ...
    internal static void SetupShaderSettingFromMaterial(Material material, ref lilToonSetting shaderSetting)
    {
        ...
        if (!shaderSetting.LIL_FEATURE_ANIMATE_MAIN_UV && material.HasProperty("_MainTex_ScrollRotate") &&
            material.GetVector("_MainTex_ScrollRotate") != lilConstants.defaultScrollRotate)
        {
            logs.Add("[lilToon] LIL_FEATURE_ANIMATE_MAIN_UV : " + AssetDatabase.GetAssetPath(material));
            shaderSetting.LIL_FEATURE_ANIMATE_MAIN_UV = true;
        }

        if (!shaderSetting.LIL_FEATURE_SHADOW && material.HasProperty("_UseShadow") &&
            material.GetFloat("_UseShadow") != 0.0f)
        {
            logs.Add("[lilToon] LIL_FEATURE_SHADOW : " + AssetDatabase.GetAssetPath(material));
            shaderSetting.LIL_FEATURE_SHADOW = true;
        }

        if (!shaderSetting.LIL_FEATURE_RIMSHADE && material.HasProperty("_UseRimShade") &&
            material.GetFloat("_UseRimShade") != 0.0f)
        {
            logs.Add("[lilToon] LIL_FEATURE_RIMSHADE : " + AssetDatabase.GetAssetPath(material));
            shaderSetting.LIL_FEATURE_RIMSHADE = true;
        }

        if (!shaderSetting.LIL_FEATURE_RECEIVE_SHADOW && material.HasProperty("_UseShadow") &&
            material.GetFloat("_UseShadow") != 0.0f && (
        ...
        if (!shaderSetting.LIL_FEATURE_DISTANCE_FADE && material.HasProperty("_DistanceFade") &&
            material.GetVector("_DistanceFade").z != lilConstants.defaultDistanceFadeParams.z)
        {
            logs.Add("[lilToon] LIL_FEATURE_DISTANCE_FADE : " + AssetDatabase.GetAssetPath(material));
            shaderSetting.LIL_FEATURE_DISTANCE_FADE = true;
        }

        if (!shaderSetting.LIL_FEATURE_SHADOW_3RD && material.HasProperty("_Shadow3rdColor") &&
            material.GetColor("_Shadow3rdColor").a != 0.0f)
        {
            logs.Add("[lilToon] LIL_FEATURE_SHADOW_3RD : " + AssetDatabase.GetAssetPath(material));
            shaderSetting.LIL_FEATURE_SHADOW_3RD = true;
        }

        if (!shaderSetting.LIL_FEATURE_SHADOW_LUT && material.HasProperty("_ShadowColorType") &&
            material.GetFloat("_ShadowColorType") != 0.0f)
        {
        ...
        if (material.shader.name.Contains("Fur"))
            if (!shaderSetting.LIL_FEATURE_FUR_COLLISION && material.HasProperty("_FurTouchStrength") &&
                material.GetFloat("_FurTouchStrength") != 0.0f)
            {
        ...
        if (!shaderSetting.LIL_FEATURE_MAIN_TONE_CORRECTION && material.HasProperty("_MainTexHSVG") &&
            material.GetVector("_MainTexHSVG") != lilConstants.defaultHSVG)
        {
            logs.Add("[lilToon] LIL_FEATURE_MAIN_TONE_CORRECTION : " + AssetDatabase.GetAssetPath(material));
            shaderSetting.LIL_FEATURE_MAIN_TONE_CORRECTION = true;
        }

        if (!shaderSetting.LIL_FEATURE_MAIN_GRADATION_MAP && material.HasProperty("_MainGradationStrength") &&
            material.GetFloat("_MainGradationStrength") != 0.0f)
        {
            logs.Add("[lilToon] LIL_FEATURE_MAIN_GRADATION_MAP : " + AssetDatabase.GetAssetPath(material));
            shaderSetting.LIL_FEATURE_MAIN_GRADATION_MAP = true;
        }

        if (!shaderSetting.LIL_FEATURE_MAIN2ND && material.HasProperty("_UseMain2ndTex") &&
            material.GetFloat("_UseMain2ndTex") != 0.0f)
        {
            logs.Add("[lilToon] LIL_FEATURE_MAIN2ND : " + AssetDatabase.GetAssetPath(material));
            shaderSetting.LIL_FEATURE_MAIN2ND = true;
        }

        if (!shaderSetting.LIL_FEATURE_MAIN3RD && material.HasProperty("_UseMain3rdTex") &&
            material.GetFloat("_UseMain3rdTex") != 0.0f)
        {
        ...
        if (!shaderSetting.LIL_FEATURE_ANIMATE_DECAL && (
                (material.HasProperty("_Main2ndTexDecalAnimation") && material.GetVector("_Main2ndTexDecalAnimation") !=
                    lilConstants.defaultDecalAnim) ||
                (material.HasProperty("_Main3rdTexDecalAnimation") && material.GetVector("_Main3rdTexDecalAnimation") !=
                    lilConstants.defaultDecalAnim))
           )
        {
        ...
        if (!shaderSetting.LIL_FEATURE_LAYER_DISSOLVE && (
                (material.HasProperty("_Main2ndDissolveParams") && material.GetVector("_Main2ndDissolveParams").x !=
                    lilConstants.defaultDissolveParams.x) ||
                (material.HasProperty("_Main3rdDissolveParams") && material.GetVector("_Main3rdDissolveParams").x !=
                    lilConstants.defaultDissolveParams.x))
           )
        {
        ...
        if (!shaderSetting.LIL_FEATURE_ALPHAMASK && material.HasProperty("_AlphaMaskMode") &&
            material.GetFloat("_AlphaMaskMode") != 0.0f)
        {
            logs.Add("[lilToon] LIL_FEATURE_ALPHAMASK : " + AssetDatabase.GetAssetPath(material));
            shaderSetting.LIL_FEATURE_ALPHAMASK = true;
        }

        if (!shaderSetting.LIL_FEATURE_EMISSION_1ST && material.HasProperty("_UseEmission") &&
            material.GetFloat("_UseEmission") != 0.0f)
        {
            logs.Add("[lilToon] LIL_FEATURE_EMISSION_1ST : " + AssetDatabase.GetAssetPath(material));
            shaderSetting.LIL_FEATURE_EMISSION_1ST = true;
        }

        if (!shaderSetting.LIL_FEATURE_EMISSION_2ND && material.HasProperty("_UseEmission2nd") &&
            material.GetFloat("_UseEmission2nd") != 0.0f)
        {
        ...
        if (!shaderSetting.LIL_FEATURE_ANIMATE_EMISSION_UV && (
                (material.HasProperty("_EmissionMap_ScrollRotate") && material.GetVector("_EmissionMap_ScrollRotate") !=
                    lilConstants.defaultScrollRotate) ||
                (material.HasProperty("_Emission2ndMap_ScrollRotate") &&
                 material.GetVector("_Emission2ndMap_ScrollRotate") != lilConstants.defaultScrollRotate))
           )
        {
        ...
        if (!shaderSetting.LIL_FEATURE_ANIMATE_EMISSION_MASK_UV && (
                (material.HasProperty("_EmissionBlendMask_ScrollRotate") &&
                 material.GetVector("_EmissionBlendMask_ScrollRotate") != lilConstants.defaultScrollRotate) ||
                (material.HasProperty("_Emission2ndBlendMask_ScrollRotate") &&
                 material.GetVector("_Emission2ndBlendMask_ScrollRotate") != lilConstants.defaultScrollRotate))
           )
        {
        ...
        if (!shaderSetting.LIL_FEATURE_NORMAL_1ST && material.HasProperty("_UseBumpMap") &&
            material.GetFloat("_UseBumpMap") != 0.0f)
        {
            logs.Add("[lilToon] LIL_FEATURE_NORMAL_1ST : " + AssetDatabase.GetAssetPath(material));
            shaderSetting.LIL_FEATURE_NORMAL_1ST = true;
        }

        if (!shaderSetting.LIL_FEATURE_NORMAL_2ND && material.HasProperty("_UseBump2ndMap") &&
            material.GetFloat("_UseBump2ndMap") != 0.0f)
        {
            logs.Add("[lilToon] LIL_FEATURE_NORMAL_2ND : " + AssetDatabase.GetAssetPath(material));
            shaderSetting.LIL_FEATURE_NORMAL_2ND = true;
        }

        if (!shaderSetting.LIL_FEATURE_ANISOTROPY && material.HasProperty("_UseAnisotropy") &&
            material.GetFloat("_UseAnisotropy") != 0.0f)
        {
            logs.Add("[lilToon] LIL_FEATURE_ANISOTROPY : " + AssetDatabase.GetAssetPath(material));
            shaderSetting.LIL_FEATURE_ANISOTROPY = true;
        }

        if (!shaderSetting.LIL_FEATURE_REFLECTION && material.HasProperty("_UseReflection") &&
            material.GetFloat("_UseReflection") != 0.0f)
        {
            logs.Add("[lilToon] LIL_FEATURE_REFLECTION : " + AssetDatabase.GetAssetPath(material));
            shaderSetting.LIL_FEATURE_REFLECTION = true;
        }

        if (!shaderSetting.LIL_FEATURE_MATCAP && material.HasProperty("_UseMatCap") &&
            material.GetFloat("_UseMatCap") != 0.0f)
        {
            logs.Add("[lilToon] LIL_FEATURE_MATCAP : " + AssetDatabase.GetAssetPath(material));
            shaderSetting.LIL_FEATURE_MATCAP = true;
        }

        if (!shaderSetting.LIL_FEATURE_MATCAP_2ND && material.HasProperty("_UseMatCap2nd") &&
            material.GetFloat("_UseMatCap2nd") != 0.0f)
        {
            logs.Add("[lilToon] LIL_FEATURE_MATCAP_2ND : " + AssetDatabase.GetAssetPath(material));
            shaderSetting.LIL_FEATURE_MATCAP_2ND = true;
        }

        if (!shaderSetting.LIL_FEATURE_RIMLIGHT && material.HasProperty("_UseRim") &&
            material.GetFloat("_UseRim") != 0.0f)
        {
            logs.Add("[lilToon] LIL_FEATURE_RIMLIGHT : " + AssetDatabase.GetAssetPath(material));
            shaderSetting.LIL_FEATURE_RIMLIGHT = true;
        }

        if (!shaderSetting.LIL_FEATURE_RIMLIGHT_DIRECTION && material.HasProperty("_RimDirStrength") &&
            material.GetFloat("_RimDirStrength") != 0.0f)
        {
            logs.Add("[lilToon] LIL_FEATURE_RIMLIGHT_DIRECTION : " + AssetDatabase.GetAssetPath(material));
            shaderSetting.LIL_FEATURE_RIMLIGHT_DIRECTION = true;
        }

        if (!shaderSetting.LIL_FEATURE_GLITTER && material.HasProperty("_UseGlitter") &&
            material.GetFloat("_UseGlitter") != 0.0f)
        {
            logs.Add("[lilToon] LIL_FEATURE_GLITTER : " + AssetDatabase.GetAssetPath(material));
            shaderSetting.LIL_FEATURE_GLITTER = true;
        }

        if (!shaderSetting.LIL_FEATURE_BACKLIGHT && material.HasProperty("_UseBacklight") &&
            material.GetFloat("_UseBacklight") != 0.0f)
        {
            logs.Add("[lilToon] LIL_FEATURE_BACKLIGHT : " + AssetDatabase.GetAssetPath(material));
            shaderSetting.LIL_FEATURE_BACKLIGHT = true;
        }

        if (!shaderSetting.LIL_FEATURE_PARALLAX && material.HasProperty("_UseParallax") &&
            material.GetFloat("_UseParallax") != 0.0f)
        {
            logs.Add("[lilToon] LIL_FEATURE_PARALLAX : " + AssetDatabase.GetAssetPath(material));
            shaderSetting.LIL_FEATURE_PARALLAX = true;
            if (!shaderSetting.LIL_FEATURE_POM && material.HasProperty("_UsePOM") &&
                material.GetFloat("_UsePOM") != 0.0f)
            {
        ...
        if (!shaderSetting.LIL_FEATURE_AUDIOLINK && material.HasProperty("_UseAudioLink") &&
            material.GetFloat("_UseAudioLink") != 0.0f)
        {
            logs.Add("[lilToon] LIL_FEATURE_AUDIOLINK : " + AssetDatabase.GetAssetPath(material));
            shaderSetting.LIL_FEATURE_AUDIOLINK = true;
        }

        if (!shaderSetting.LIL_FEATURE_AUDIOLINK_VERTEX && material.HasProperty("_AudioLink2Vertex") &&
            material.GetFloat("_AudioLink2Vertex") != 0.0f)
        {
            logs.Add("[lilToon] LIL_FEATURE_AUDIOLINK_VERTEX : " + AssetDatabase.GetAssetPath(material));
            shaderSetting.LIL_FEATURE_AUDIOLINK_VERTEX = true;
        }

        if (!shaderSetting.LIL_FEATURE_AUDIOLINK_LOCAL && material.HasProperty("_AudioLinkAsLocal") &&
            material.GetFloat("_AudioLinkAsLocal") != 0.0f)
        {
            logs.Add("[lilToon] LIL_FEATURE_AUDIOLINK_LOCAL : " + AssetDatabase.GetAssetPath(material));
            shaderSetting.LIL_FEATURE_AUDIOLINK_LOCAL = true;
        }

        if (!shaderSetting.LIL_FEATURE_DISSOLVE && material.HasProperty("_DissolveParams") &&
            material.GetVector("_DissolveParams").x != lilConstants.defaultDissolveParams.x)
        {
            logs.Add("[lilToon] LIL_FEATURE_DISSOLVE : " + AssetDatabase.GetAssetPath(material));
            shaderSetting.LIL_FEATURE_DISSOLVE = true;
        }

        if (!shaderSetting.LIL_FEATURE_DITHER && material.HasProperty("_UseDither") &&
            material.GetFloat("_UseDither") != 0.0f)
        {
        ...
        if (!shaderSetting.LIL_FEATURE_UDIMDISCARD && material.HasProperty("_UDIMDiscardCompile") &&
            material.GetFloat("_UDIMDiscardCompile") != 0.0f)
        {
        ...
        {
            if (!shaderSetting.LIL_FEATURE_ANIMATE_OUTLINE_UV && material.HasProperty("_OutlineTex_ScrollRotate") &&
                material.GetVector("_OutlineTex_ScrollRotate") != lilConstants.defaultScrollRotate)
            {
                logs.Add("[lilToon] LIL_FEATURE_ANIMATE_OUTLINE_UV : " + AssetDatabase.GetAssetPath(material));
                shaderSetting.LIL_FEATURE_ANIMATE_OUTLINE_UV = true;
            }

            if (!shaderSetting.LIL_FEATURE_OUTLINE_TONE_CORRECTION && material.HasProperty("_OutlineTexHSVG") &&
                material.GetVector("_OutlineTexHSVG") != lilConstants.defaultHSVG)
            {
                logs.Add("[lilToon] LIL_FEATURE_OUTLINE_TONE_CORRECTION : " + AssetDatabase.GetAssetPath(material));
                shaderSetting.LIL_FEATURE_OUTLINE_TONE_CORRECTION = true;
            }

            if (!shaderSetting.LIL_FEATURE_OUTLINE_RECEIVE_SHADOW && material.HasProperty("_OutlineLitShadowReceive") &&
                material.GetFloat("_OutlineLitShadowReceive") != 0.0f)
            {
        ...
        if (!shaderSetting.LIL_OPTIMIZE_USE_VERTEXLIGHT && material.HasProperty("_VertexLightStrength") &&
            material.GetFloat("_VertexLightStrength") != 0.0f)
        {
            logs.Add("[lilToon] LIL_OPTIMIZE_USE_VERTEXLIGHT : " + AssetDatabase.GetAssetPath(material));
            shaderSetting.LIL_OPTIMIZE_USE_VERTEXLIGHT = true;
        }
    ...
    private static void SetupShaderSettingFromAnimationClip(AnimationClip clip, ref lilToonSetting shaderSetting,
        bool shouldCheckMaterial = false)
    {
        if (clip == null) return;

        if (shouldCheckMaterial)
            foreach (var frame in AnimationUtility.GetObjectReferenceCurveBindings(clip)
                         .SelectMany(b => AnimationUtility.GetObjectReferenceCurve(clip, b))
                         .Where(f => f.value is Material))
        ...
        foreach (var binding in AnimationUtility.GetCurveBindings(clip))
        {
            ...
            shaderSetting.LIL_FEATURE_ANIMATE_MAIN_UV = shaderSetting.LIL_FEATURE_ANIMATE_MAIN_UV ||
                                                        propname.Contains("_MainTex_ScrollRotate");
            shaderSetting.LIL_FEATURE_SHADOW = shaderSetting.LIL_FEATURE_SHADOW || propname.Contains("_UseShadow");
            shaderSetting.LIL_FEATURE_RECEIVE_SHADOW = shaderSetting.LIL_FEATURE_RECEIVE_SHADOW ||
                                                       propname.Contains("_ShadowReceive") ||
                                                       propname.Contains("_Shadow2ndReceive") ||
                                                       propname.Contains("_Shadow3rdReceive");
            shaderSetting.LIL_FEATURE_DISTANCE_FADE =
                shaderSetting.LIL_FEATURE_DISTANCE_FADE || propname.Contains("_DistanceFade");
            shaderSetting.LIL_FEATURE_SHADOW_3RD =
                shaderSetting.LIL_FEATURE_SHADOW_3RD || propname.Contains("_Shadow3rdColor");
            shaderSetting.LIL_FEATURE_SHADOW_LUT =
                shaderSetting.LIL_FEATURE_SHADOW_LUT || propname.Contains("_ShadowColorType");
            shaderSetting.LIL_FEATURE_RIMSHADE =
                shaderSetting.LIL_FEATURE_RIMSHADE || propname.Contains("_UseRimShade");

            shaderSetting.LIL_FEATURE_FUR_COLLISION =
                shaderSetting.LIL_FEATURE_FUR_COLLISION || propname.Contains("_FurTouchStrength");

            shaderSetting.LIL_FEATURE_MAIN_TONE_CORRECTION =
                shaderSetting.LIL_FEATURE_MAIN_TONE_CORRECTION || propname.Contains("_MainTexHSVG");
            shaderSetting.LIL_FEATURE_MAIN_GRADATION_MAP = shaderSetting.LIL_FEATURE_MAIN_GRADATION_MAP ||
                                                           propname.Contains("_MainGradationStrength");
            shaderSetting.LIL_FEATURE_MAIN2ND =
                shaderSetting.LIL_FEATURE_MAIN2ND || propname.Contains("_UseMain2ndTex");
            shaderSetting.LIL_FEATURE_MAIN3RD =
                shaderSetting.LIL_FEATURE_MAIN3RD || propname.Contains("_UseMain3rdTex");
            shaderSetting.LIL_FEATURE_DECAL = shaderSetting.LIL_FEATURE_DECAL ||
                                              propname.Contains("_Main2ndTexIsDecal") ||
                                              propname.Contains("_Main3rdTexIsDecal");
            shaderSetting.LIL_FEATURE_ANIMATE_DECAL = shaderSetting.LIL_FEATURE_ANIMATE_DECAL ||
                                                      propname.Contains("_Main2ndTexDecalAnimation") ||
                                                      propname.Contains("_Main3rdTexDecalAnimation");
            shaderSetting.LIL_FEATURE_LAYER_DISSOLVE = shaderSetting.LIL_FEATURE_LAYER_DISSOLVE ||
                                                       propname.Contains("_Main2ndDissolveParams") ||
                                                       propname.Contains("_Main3rdDissolveParams");
            shaderSetting.LIL_FEATURE_ALPHAMASK =
                shaderSetting.LIL_FEATURE_ALPHAMASK || propname.Contains("_AlphaMaskMode");
            shaderSetting.LIL_FEATURE_EMISSION_1ST =
                shaderSetting.LIL_FEATURE_EMISSION_1ST || propname.Contains("_UseEmission");
            shaderSetting.LIL_FEATURE_EMISSION_2ND =
                shaderSetting.LIL_FEATURE_EMISSION_2ND || propname.Contains("_UseEmission2nd");
            shaderSetting.LIL_FEATURE_ANIMATE_EMISSION_UV = shaderSetting.LIL_FEATURE_ANIMATE_EMISSION_UV ||
                                                            propname.Contains("_EmissionMap_ScrollRotate") ||
                                                            propname.Contains("_Emission2ndMap_ScrollRotate");
            shaderSetting.LIL_FEATURE_ANIMATE_EMISSION_MASK_UV = shaderSetting.LIL_FEATURE_ANIMATE_EMISSION_MASK_UV ||
                                                                 propname.Contains("_EmissionBlendMask_ScrollRotate") ||
                                                                 propname.Contains(
                                                                     "_Emission2ndBlendMask_ScrollRotate");
            shaderSetting.LIL_FEATURE_EMISSION_GRADATION = shaderSetting.LIL_FEATURE_EMISSION_GRADATION ||
                                                           propname.Contains("_EmissionUseGrad");
            shaderSetting.LIL_FEATURE_NORMAL_1ST =
                shaderSetting.LIL_FEATURE_NORMAL_1ST || propname.Contains("_UseBumpMap");
            shaderSetting.LIL_FEATURE_NORMAL_2ND =
                shaderSetting.LIL_FEATURE_NORMAL_2ND || propname.Contains("_UseBump2ndMap");
            shaderSetting.LIL_FEATURE_ANISOTROPY =
                shaderSetting.LIL_FEATURE_ANISOTROPY || propname.Contains("_UseAnisotropy");
            shaderSetting.LIL_FEATURE_REFLECTION =
                shaderSetting.LIL_FEATURE_REFLECTION || propname.Contains("_UseReflection");
            shaderSetting.LIL_FEATURE_MATCAP = shaderSetting.LIL_FEATURE_MATCAP || propname.Contains("_UseMatCap");
            shaderSetting.LIL_FEATURE_MATCAP_2ND =
                shaderSetting.LIL_FEATURE_MATCAP_2ND || propname.Contains("_UseMatCap2nd");
            ...
            shaderSetting.LIL_FEATURE_BACKLIGHT =
                shaderSetting.LIL_FEATURE_BACKLIGHT || propname.Contains("_UseBacklight");
            shaderSetting.LIL_FEATURE_RIMLIGHT_DIRECTION = shaderSetting.LIL_FEATURE_RIMLIGHT_DIRECTION ||
                                                           propname.Contains("_RimDirStrength");
            shaderSetting.LIL_FEATURE_PARALLAX =
                shaderSetting.LIL_FEATURE_PARALLAX || propname.Contains("_UseParallax");
            shaderSetting.LIL_FEATURE_POM = shaderSetting.LIL_FEATURE_POM || propname.Contains("_UsePOM");
            shaderSetting.LIL_FEATURE_AUDIOLINK =
                shaderSetting.LIL_FEATURE_AUDIOLINK || propname.Contains("_UseAudioLink");
            shaderSetting.LIL_FEATURE_AUDIOLINK_VERTEX =
                shaderSetting.LIL_FEATURE_AUDIOLINK_VERTEX || propname.Contains("_AudioLink2Vertex");
            shaderSetting.LIL_FEATURE_AUDIOLINK_LOCAL =
                shaderSetting.LIL_FEATURE_AUDIOLINK_LOCAL || propname.Contains("_AudioLinkAsLocal");
            shaderSetting.LIL_FEATURE_DISSOLVE =
                shaderSetting.LIL_FEATURE_DISSOLVE || propname.Contains("_DissolveParams");
            ...
            if (!shaderSetting.LIL_FEATURE_IDMASK && (
                    ...
                    propname.Contains("_IDMaskPrior1") ||
                    propname.Contains("_IDMask2") || propname.Contains("_IDMaskIndex2") ||
                    propname.Contains("_IDMaskPrior2") ||
                    propname.Contains("_IDMask3") || propname.Contains("_IDMaskIndex3") ||
                    propname.Contains("_IDMaskPrior3") ||
                    propname.Contains("_IDMask4") || propname.Contains("_IDMaskIndex4") ||
                    propname.Contains("_IDMaskPrior4") ||
                    propname.Contains("_IDMask5") || propname.Contains("_IDMaskIndex5") ||
                    propname.Contains("_IDMaskPrior5") ||
                    propname.Contains("_IDMask6") || propname.Contains("_IDMaskIndex6") ||
                    propname.Contains("_IDMaskPrior6") ||
                    propname.Contains("_IDMask7") || propname.Contains("_IDMaskIndex7") ||
                    propname.Contains("_IDMaskPrior7") ||
                    propname.Contains("_IDMask8") || propname.Contains("_IDMaskIndex8") ||
                    propname.Contains("_IDMaskPrior8")
                ))
                shaderSetting.LIL_FEATURE_IDMASK = true;

            shaderSetting.LIL_FEATURE_UDIMDISCARD =
                shaderSetting.LIL_FEATURE_UDIMDISCARD || propname.Contains("_UDIMDiscardCompile");

            shaderSetting.LIL_FEATURE_ENCRYPTION =
                shaderSetting.LIL_FEATURE_ENCRYPTION || propname.Contains("_BitKey0");

            shaderSetting.LIL_FEATURE_ANIMATE_OUTLINE_UV = shaderSetting.LIL_FEATURE_ANIMATE_OUTLINE_UV ||
                                                           propname.Contains("_OutlineTex_ScrollRotate");
            shaderSetting.LIL_FEATURE_OUTLINE_TONE_CORRECTION = shaderSetting.LIL_FEATURE_OUTLINE_TONE_CORRECTION ||
                                                                propname.Contains("_OutlineTexHSVG");
            shaderSetting.LIL_FEATURE_OUTLINE_RECEIVE_SHADOW = shaderSetting.LIL_FEATURE_OUTLINE_RECEIVE_SHADOW ||
                                                               propname.Contains("_OutlineLitShadowReceive");

            shaderSetting.LIL_OPTIMIZE_USE_VERTEXLIGHT = shaderSetting.LIL_OPTIMIZE_USE_VERTEXLIGHT ||
                                                         shaderSetting.LIL_FEATURE_FUR_COLLISION ||
                                                         propname.Contains("_VertexLightStrength");
    ...
    internal static void CheckTextures(ref lilToonSetting shaderSetting)
    {
        ...
        // Get animations
        foreach (var propname in lilDirectoryManager.FindAssets<AnimationClip>("t:animationclip")
                     .SelectMany(c => AnimationUtility.GetCurveBindings(c)).Select(b => b.propertyName)
                     .Where(n => !string.IsNullOrEmpty(n) && n.Contains("material.")))
            CheckTextures(ref shaderSetting, propname);
    }
    ...
    {
        shaderSetting.LIL_FEATURE_MainGradationTex =
            shaderSetting.LIL_FEATURE_MainGradationTex || propname.Contains("_MainGradationTex");
        shaderSetting.LIL_FEATURE_MainColorAdjustMask = shaderSetting.LIL_FEATURE_MainColorAdjustMask ||
                                                        propname.Contains("_MainColorAdjustMask");
        shaderSetting.LIL_FEATURE_Main2ndTex = shaderSetting.LIL_FEATURE_Main2ndTex || propname.Contains("_Main2ndTex");
        shaderSetting.LIL_FEATURE_Main2ndBlendMask =
            shaderSetting.LIL_FEATURE_Main2ndBlendMask || propname.Contains("_Main2ndBlendMask");
        shaderSetting.LIL_FEATURE_Main2ndDissolveMask = shaderSetting.LIL_FEATURE_Main2ndDissolveMask ||
                                                        propname.Contains("_Main2ndDissolveMask");
        shaderSetting.LIL_FEATURE_Main2ndDissolveNoiseMask = shaderSetting.LIL_FEATURE_Main2ndDissolveNoiseMask ||
                                                             propname.Contains("_Main2ndDissolveNoiseMask");
        ...
        shaderSetting.LIL_FEATURE_Main3rdBlendMask =
            shaderSetting.LIL_FEATURE_Main3rdBlendMask || propname.Contains("_Main3rdBlendMask");
        shaderSetting.LIL_FEATURE_Main3rdDissolveMask = shaderSetting.LIL_FEATURE_Main3rdDissolveMask ||
                                                        propname.Contains("_Main3rdDissolveMask");
        shaderSetting.LIL_FEATURE_Main3rdDissolveNoiseMask = shaderSetting.LIL_FEATURE_Main3rdDissolveNoiseMask ||
                                                             propname.Contains("_Main3rdDissolveNoiseMask");
        ...
        shaderSetting.LIL_FEATURE_Bump2ndScaleMask =
            shaderSetting.LIL_FEATURE_Bump2ndScaleMask || propname.Contains("_Bump2ndScaleMask");
        shaderSetting.LIL_FEATURE_AnisotropyTangentMap = shaderSetting.LIL_FEATURE_AnisotropyTangentMap ||
                                                         propname.Contains("_AnisotropyTangentMap");
        shaderSetting.LIL_FEATURE_AnisotropyScaleMask = shaderSetting.LIL_FEATURE_AnisotropyScaleMask ||
                                                        propname.Contains("_AnisotropyScaleMask");
        shaderSetting.LIL_FEATURE_AnisotropyShiftNoiseMask = shaderSetting.LIL_FEATURE_AnisotropyShiftNoiseMask ||
                                                             propname.Contains("_AnisotropyShiftNoiseMask");
        shaderSetting.LIL_FEATURE_ShadowBorderMask =
            shaderSetting.LIL_FEATURE_ShadowBorderMask || propname.Contains("_ShadowBorderMask");
        shaderSetting.LIL_FEATURE_ShadowBlurMask =
            shaderSetting.LIL_FEATURE_ShadowBlurMask || propname.Contains("_ShadowBlurMask");
        shaderSetting.LIL_FEATURE_ShadowStrengthMask = shaderSetting.LIL_FEATURE_ShadowStrengthMask ||
                                                       propname.Contains("_ShadowStrengthMask");
        shaderSetting.LIL_FEATURE_ShadowColorTex =
            shaderSetting.LIL_FEATURE_ShadowColorTex || propname.Contains("_ShadowColorTex");
        shaderSetting.LIL_FEATURE_Shadow2ndColorTex =
            shaderSetting.LIL_FEATURE_Shadow2ndColorTex || propname.Contains("_Shadow2ndColorTex");
        shaderSetting.LIL_FEATURE_Shadow3rdColorTex =
            shaderSetting.LIL_FEATURE_Shadow3rdColorTex || propname.Contains("_Shadow3rdColorTex");
        shaderSetting.LIL_FEATURE_RimShadeMask =
            shaderSetting.LIL_FEATURE_RimShadeMask || propname.Contains("_RimShadeMask");
        shaderSetting.LIL_FEATURE_BacklightColorTex =
            shaderSetting.LIL_FEATURE_BacklightColorTex || propname.Contains("_BacklightColorTex");
        shaderSetting.LIL_FEATURE_SmoothnessTex =
            shaderSetting.LIL_FEATURE_SmoothnessTex || propname.Contains("_SmoothnessTex");
        shaderSetting.LIL_FEATURE_MetallicGlossMap =
            shaderSetting.LIL_FEATURE_MetallicGlossMap || propname.Contains("_MetallicGlossMap");
        shaderSetting.LIL_FEATURE_ReflectionColorTex = shaderSetting.LIL_FEATURE_ReflectionColorTex ||
                                                       propname.Contains("_ReflectionColorTex");
        shaderSetting.LIL_FEATURE_ReflectionCubeTex =
            shaderSetting.LIL_FEATURE_ReflectionCubeTex || propname.Contains("_ReflectionCubeTex");
        ...
        shaderSetting.LIL_FEATURE_MatCapBlendMask =
            shaderSetting.LIL_FEATURE_MatCapBlendMask || propname.Contains("_MatCapBlendMask");
        shaderSetting.LIL_FEATURE_MatCapBumpMap =
            shaderSetting.LIL_FEATURE_MatCapBumpMap || propname.Contains("_MatCapBumpMap");
        shaderSetting.LIL_FEATURE_MatCap2ndTex =
            shaderSetting.LIL_FEATURE_MatCap2ndTex || propname.Contains("_MatCap2ndTex");
        shaderSetting.LIL_FEATURE_MatCap2ndBlendMask = shaderSetting.LIL_FEATURE_MatCap2ndBlendMask ||
                                                       propname.Contains("_MatCap2ndBlendMask");
        shaderSetting.LIL_FEATURE_MatCap2ndBumpMap =
            shaderSetting.LIL_FEATURE_MatCap2ndBumpMap || propname.Contains("_MatCap2ndBumpMap");
        shaderSetting.LIL_FEATURE_RimColorTex =
            shaderSetting.LIL_FEATURE_RimColorTex || propname.Contains("_RimColorTex");
        shaderSetting.LIL_FEATURE_GlitterColorTex =
            shaderSetting.LIL_FEATURE_GlitterColorTex || propname.Contains("_GlitterColorTex");
        shaderSetting.LIL_FEATURE_GlitterShapeTex =
            shaderSetting.LIL_FEATURE_GlitterShapeTex || propname.Contains("_GlitterShapeTex");
        shaderSetting.LIL_FEATURE_EmissionMap =
            shaderSetting.LIL_FEATURE_EmissionMap || propname.Contains("_EmissionMap");
        shaderSetting.LIL_FEATURE_EmissionBlendMask =
            shaderSetting.LIL_FEATURE_EmissionBlendMask || propname.Contains("_EmissionBlendMask");
        shaderSetting.LIL_FEATURE_EmissionGradTex =
            shaderSetting.LIL_FEATURE_EmissionGradTex || propname.Contains("_EmissionGradTex");
        shaderSetting.LIL_FEATURE_Emission2ndMap =
            shaderSetting.LIL_FEATURE_Emission2ndMap || propname.Contains("_Emission2ndMap");
        shaderSetting.LIL_FEATURE_Emission2ndBlendMask = shaderSetting.LIL_FEATURE_Emission2ndBlendMask ||
                                                         propname.Contains("_Emission2ndBlendMask");
        shaderSetting.LIL_FEATURE_Emission2ndGradTex = shaderSetting.LIL_FEATURE_Emission2ndGradTex ||
                                                       propname.Contains("_Emission2ndGradTex");
        shaderSetting.LIL_FEATURE_ParallaxMap =
            shaderSetting.LIL_FEATURE_ParallaxMap || propname.Contains("_ParallaxMap");
        shaderSetting.LIL_FEATURE_AudioLinkMask =
            shaderSetting.LIL_FEATURE_AudioLinkMask || propname.Contains("_AudioLinkMask");
        shaderSetting.LIL_FEATURE_AudioLinkLocalMap =
            shaderSetting.LIL_FEATURE_AudioLinkLocalMap || propname.Contains("_AudioLinkLocalMap");
        shaderSetting.LIL_FEATURE_DissolveMask =
            shaderSetting.LIL_FEATURE_DissolveMask || propname.Contains("_DissolveMask");
        shaderSetting.LIL_FEATURE_DissolveNoiseMask =
            shaderSetting.LIL_FEATURE_DissolveNoiseMask || propname.Contains("_DissolveNoiseMask");
        ...
        shaderSetting.LIL_FEATURE_OutlineWidthMask =
            shaderSetting.LIL_FEATURE_OutlineWidthMask || propname.Contains("_OutlineWidthMask");
        shaderSetting.LIL_FEATURE_OutlineVectorTex =
            shaderSetting.LIL_FEATURE_OutlineVectorTex || propname.Contains("_OutlineVectorTex");
        shaderSetting.LIL_FEATURE_FurNoiseMask =
            shaderSetting.LIL_FEATURE_FurNoiseMask || propname.Contains("_FurNoiseMask");
        ...
        shaderSetting.LIL_FEATURE_FurLengthMask =
            shaderSetting.LIL_FEATURE_FurLengthMask || propname.Contains("_FurLengthMask");
        shaderSetting.LIL_FEATURE_FurVectorTex =
            shaderSetting.LIL_FEATURE_FurVectorTex || propname.Contains("_FurVectorTex");
    }
    ...
    {
        return GetTrueShaderLists(lilDirectoryManager.FindAssets<Material>("t:material")
            .Where(m => lilMaterialUtils.CheckShaderIslilToon(m)).Select(m => m.shader).Distinct().ToList());
    }
    ...
    private static List<Shader> GetShaderListFromGameObject(Material[] materials, AnimationClip[] clips)
    {
        ...
        foreach (var clip in clips) CheckAnimationClip(clip, shaders);
    ...
    private static List<Shader> GetTrueShaderLists(List<Shader> shaders)
    {
                     ...
                     .ToArray())
        {
            ...
            while ((line = sr.ReadLine()) != null)
            {
                ...
                var second = line.IndexOf('"', first);
                if (line.Substring(0, first).Contains("//")) continue;
                var shaderName = line.Substring(first, second - first);
                var passNameSep = shaderName.LastIndexOf('/');
    ...
    {
        shaders.AddRange(AnimationUtility.GetObjectReferenceCurveBindings(clip)
            .SelectMany(b => AnimationUtility.GetObjectReferenceCurve(clip, b))
            .Where(f => lilMaterialUtils.CheckShaderIslilToon(f.value as Material))
            .Select(f => ((Material)f.value).shader));
    }
}
#endif
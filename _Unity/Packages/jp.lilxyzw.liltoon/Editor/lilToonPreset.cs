...
public class lilToonPreset : ScriptableObject
{
    ...
    public static void ApplyPreset(Material material, lilToonPreset preset, bool ismulti)
    {
        ...
        foreach (var f in preset.floats.Where(f => f.name == "_StencilPass")) material.SetFloat(f.name, f.value);
        if (preset.shader != null) material.shader = preset.shader;
        var isoutl = preset.outline == -1 ? material.shader.name.Contains("Outline") : preset.outline == 1;
        var istess = preset.tessellation == -1
            ? material.shader.name.Contains("Tessellation")
            : preset.tessellation == 1;
        ...
        var isblur = material.shader.name.Contains("Blur");
        ...
        lilMaterialUtils.SetupMaterialWithRenderingMode(material, renderingMode, transparentMode, isoutl, islite,
            istess, ismulti);
    ...
    public class lilPresetWindow : EditorWindow
    {
        ...
        private bool shouldSaveMainTex2Outline;
        ...
        // Texture
        private readonly HashSet<string> shouldSaveTexs = new();
...
#pragma warning disable CS0612
        private void OnGUI()
        {
            ...
            preset.category =
                (lilPresetCategory)EditorGUILayout.Popup(GetLoc("sPresetCategory"), (int)preset.category, sCategorys);
            ...
            if (isShowFeatures)
            {
                ...
                shouldSaveMainTex2Outline =
                    EditorGUILayout.ToggleLeft(GetLoc("sPresetMainTex2Outline"), shouldSaveMainTex2Outline);
                ...
                )
                {
                    ...
                    shouldSaveFurRendering =
                        EditorGUILayout.ToggleLeft(GetLoc("sRenderingSetting") + " - " + GetLoc("sFur"),
                            shouldSaveFurRendering);
                }
                else
                {
                ...
                shouldSaveOutlineRendering =
                    EditorGUILayout.ToggleLeft(GetLoc("sOutline") + " - " + GetLoc("sRenderingSetting"),
                        shouldSaveOutlineRendering);
                EditorGUI.indentLevel--;
            ...
            if (isShowTextures)
            {
                ...
                for (var i = 0; i < propCount; i++)
                {
                    ...
                    var shouldSave = shouldSaveTexs.Contains(propName);
            ...
            if (GUILayout.Button("Save"))
            {
                ...
                for (var i = 0; i < langName.Length; i++)
                    if (!string.IsNullOrEmpty(presetName[i]))
                    {
                        ...
                        if (string.IsNullOrEmpty(filename) || langName[i] == "English")
                            filename = preset.category + "-" + presetName[i];
                    }
                ...
                var presetFolderPath = lilDirectoryManager.GetPresetsFolderPath();
                if (presetFolderPath.Contains("Packages")) presetFolderPath = "Assets/";
                var savePath = EditorUtility.SaveFilePanel("Save Preset", presetFolderPath, filename, "asset");
                if (!string.IsNullOrEmpty(savePath))
                {
                    AssetDatabase.CreateAsset(preset, FileUtil.GetProjectRelativePath(savePath));
                    AssetDatabase.SaveAssets();
                    AssetDatabase.Refresh();
                    AssetDatabase.ImportAsset(FileUtil.GetProjectRelativePath(savePath),
...
#pragma warning restore CS0612
        ...
        private void CopyPropertiesToPreset(Material material)
        {
            ...
            for (var i = 0; i < propCount; i++)
            {
                ...
                if (propType == ShaderUtil.ShaderPropertyType.Float || propType == ShaderUtil.ShaderPropertyType.Range)
                    if (!(!shouldSaveStencil && propName == "_StencilRef" && propName == "_StencilComp" &&
                          propName == "_StencilPass" && propName == "_StencilFail" && propName == "_StencilZFail"))
                    {
        ...
        {
            foreach (var name in shouldSaveTexs) CopyTextureToPreset(material, name);
        }

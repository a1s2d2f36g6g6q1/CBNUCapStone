...
namespace lilToon
{
    public partial class lilToonInspector
    {
        ...
        private void TextureBake(Material material, int bakeType)
        {
            ...
            var shouldNotBakeColor = (bakeType == 1 || bakeType == 4) && mainColor.colorValue == Color.white &&
                                     mainTexHSVG.vectorValue == lilConstants.defaultHSVG &&
                                     mainGradationStrength.floatValue == 0.0;
            var cannotBake1st = mainTex.textureValue == null;
            var shouldNotBake2nd = (bakeType == 2 || bakeType == 5) && useMain2ndTex.floatValue == 0.0;
            var shouldNotBake3rd = (bakeType == 3 || bakeType == 6) && useMain3rdTex.floatValue == 0.0;
            var shouldNotBakeAll = bakeType == 0 && mainColor.colorValue == Color.white &&
                                   mainTexHSVG.vectorValue == lilConstants.defaultHSVG &&
                                   mainGradationStrength.floatValue == 0.0 && useMain2ndTex.floatValue == 0.0 &&
            ...
            {
                var bake2nd = (bakeType == 0 || bakeType == 2 || bakeType == 5) && useMain2ndTex.floatValue != 0.0;
                var bake3rd = (bakeType == 0 || bakeType == 3 || bakeType == 6) && useMain3rdTex.floatValue != 0.0;
                // run bake
                ...
                if (bake2nd)
                {
                    ...
                    hsvgMaterial.SetTextureOffset(main2ndBlendMask.name,
                        material.GetTextureOffset(main2ndBlendMask.name));
                    hsvgMaterial.SetTextureScale(main2ndBlendMask.name,
                ...
                if (bake3rd)
                {
                    ...
                    hsvgMaterial.SetTextureOffset(main3rdBlendMask.name,
                        material.GetTextureOffset(main3rdBlendMask.name));
                    hsvgMaterial.SetTextureScale(main3rdBlendMask.name,
                        material.GetTextureScale(main3rdBlendMask.name));
        ...
        {
            var shouldNotBakeAll = mainColor.colorValue == Color.white &&
                                   mainTexHSVG.vectorValue == lilConstants.defaultHSVG &&
                                   mainGradationStrength.floatValue == 0.0 && useMain2ndTex.floatValue == 0.0 &&
                                   useMain3rdTex.floatValue == 0.0;
            if (!shouldNotBakeAll && EditorUtility.DisplayDialog(GetLoc("sDialogRunBake"), GetLoc("sDialogBakeMain"),
                    GetLoc("sYes"), GetLoc("sNo")))
            {
                var bake2nd = useMain2ndTex.floatValue != 0.0;
                var bake3rd = useMain3rdTex.floatValue != 0.0;
                // run bake
                var bufMainTexture = mainTex.textureValue as Texture2D;
                var hsvgMaterial = new Material(ltsbaker);
                ...
                outTexture = lilTextureUtils.SaveTextureToPng(material, outTexture, mainTex.name);
                if (outTexture != mainTex.textureValue) CopyTextureSetting(bufMainTexture, outTexture);
        ...
        private Texture AutoBakeShadowTexture(Material material, Texture bakedMainTex, int shadowType = 0,
            bool shouldShowDialog = true)
        {
            var shouldNotBakeAll = useShadow.floatValue == 0.0 && shadowColor.colorValue == Color.white &&
                                   shadowColorTex.textureValue == null && shadowStrengthMask.textureValue == null;
            var shouldBake = true;
            if (shouldShowDialog)
                shouldBake = EditorUtility.DisplayDialog(GetLoc("sDialogRunBake"), GetLoc("sDialogBakeShadow"),
                    GetLoc("sYes"), GetLoc("sNo"));
            if (!shouldNotBakeAll && shouldBake)
            {
                ...
                if (shadowType == 2)
                {
                    hsvgMaterial.SetColor(mainColor2nd.name,
                        new Color(shadow2ndColor.colorValue.r, shadow2ndColor.colorValue.g, shadow2ndColor.colorValue.b,
                            shadow2ndColor.colorValue.a * shadowStrength.floatValue));
                ...
                {
                    hsvgMaterial.SetColor(mainColor3rd.name,
                        new Color(shadow3rdColor.colorValue.r, shadow3rdColor.colorValue.g, shadow3rdColor.colorValue.b,
                            shadow3rdColor.colorValue.a * shadowStrength.floatValue));
                ...
                {
                    hsvgMaterial.SetColor(mainColor2nd.name,
                        new Color(shadowColor.colorValue.r, shadowColor.colorValue.g, shadowColor.colorValue.b,
                            shadowStrength.floatValue));
                    hsvgMaterial.SetFloat(main2ndTexBlendMode.name, 0.0f);
                    hsvgMaterial.SetFloat(main2ndTexAlphaMode.name, 0.0f);
                    path = AssetDatabase.GetAssetPath(material.GetTexture(shadowColorTex.name));
                }

                var existsShadowTex = !string.IsNullOrEmpty(path);
                if (existsShadowTex)
                {
                    lilTextureUtils.LoadTexture(ref srcMain2, path);
                    hsvgMaterial.SetTexture(main2ndTex.name, srcMain2);
                ...
                if (shadowType == 1)
                    outTexture = lilTextureUtils.SaveTextureToPng(material, outTexture, mainTex.name, "_shadow_1st");
                if (shadowType == 2)
                    outTexture = lilTextureUtils.SaveTextureToPng(material, outTexture, mainTex.name, "_shadow_2nd");
                if (outTexture != mainTex.textureValue) CopyTextureSetting(bufMainTexture, outTexture);
        ...
        {
            var shouldNotBakeAll = matcapColor.colorValue == Color.white;
            if (!shouldNotBakeAll && EditorUtility.DisplayDialog(GetLoc("sDialogRunBake"), GetLoc("sDialogBakeMatCap"),
                    GetLoc("sYes"), GetLoc("sNo")))
            {
                // run bake
                ...
                if (outTexture != matcapTex.textureValue) CopyTextureSetting(bufMainTexture, outTexture);
        ...
        {
            var shouldNotBakeAll = matcapBlendMask.textureValue == null && rimColorTex.textureValue == null &&
                                   emissionBlendMask.textureValue == null;
            if (!shouldNotBakeAll && EditorUtility.DisplayDialog(GetLoc("sDialogRunBake"), GetLoc("sDialogBakeTriMask"),
                    GetLoc("sYes"), GetLoc("sNo")))
            {
                // run bake
                var bufMainTexture = mainTex.textureValue as Texture2D;
        ...
        private Texture AutoBakeAlphaMask(Material material)
        {
            ...
            {
                CopyTextureSetting(bufMainTexture, outTexture);
                var savePath = AssetDatabase.GetAssetPath(outTexture);
        ...
        {
            var shouldNotBakeOutline =
                outlineTex.textureValue == null || outlineTexHSVG.vectorValue == lilConstants.defaultHSVG;
            if (!shouldNotBakeOutline && EditorUtility.DisplayDialog(GetLoc("sDialogRunBake"),
                    GetLoc("sDialogBakeOutline"), GetLoc("sYes"), GetLoc("sNo")))
            {
                // run bake
        ...
        private void AutoBakeColoredMask(Material material, MaterialProperty masktex, MaterialProperty maskcolor,
            string propName)
        {
            if (propName.Contains("Shadow"))
            {
                var shadowType = propName.Contains("2nd") ? 2 : 1;
            ...
            if (outTexture != bufMainTexture) CopyTextureSetting(bufMainTexture, outTexture);

            Object.DestroyImmediate(hsvgMaterial);
            Object.DestroyImmediate(srcTexture);
        }

        public static void RunBake(ref Texture2D outTexture, Texture2D srcTexture, Material material,
        ...
        private void CopyTextureSetting(Texture2D fromTexture, Texture2D toTexture)
        {
            ...
            var fromPath = AssetDatabase.GetAssetPath(fromTexture);
            var toPath = AssetDatabase.GetAssetPath(toTexture);

...
public class PuzzleManager : MonoBehaviour
{
    ...
    private void TriggerGameClear()
    {
        ...
        StartCoroutine(SendGameComplete());
    }
    ...
    private IEnumerator SendGameComplete()
    {
        // API: POST /games/complete
        // 보낼 데이터: roomId, 시작시간, 종료시간

        var requestData = new
        {
            roomId = GameData.roomId, // PuzzleLoader에서 저장한 roomId
                                      // startTime, endTime 추가 필요
        };
        ...
        UnityWebRequest request = new UnityWebRequest(
            "http://13.209.33.42/games/complete",
            "POST"
        );

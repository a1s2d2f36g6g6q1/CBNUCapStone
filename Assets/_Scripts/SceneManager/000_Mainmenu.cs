...
public class MainMenuUIController : MonoBehaviour
{
    ...
    private IEnumerator SendIDCheckRequest()
    {
        ...
        Debug.Log($"[ID 체크] URL: http://13.209.33.42/api/user/check-id");
        Debug.Log($"[ID 체크] 요청 본문: {json}");

        byte[] bodyRaw = Encoding.UTF8.GetBytes(json);

        UnityWebRequest request = new UnityWebRequest(
            "http://13.209.33.42/api/user/check-id",
            "POST"
        );
        ...
        Debug.Log($"[ID 체크] 응답 코드: {request.responseCode}");
        Debug.Log($"[ID 체크] 에러: {request.error}");
        Debug.Log($"[ID 체크] 응답 본문: {request.downloadHandler?.text}");

        if (request.result == UnityWebRequest.Result.Success)
        {
            var response = JsonUtility.FromJson<IDCheckResponse>(request.downloadHandler.text);
    ...
    private IEnumerator SendLoginRequest()
    {
        ...
        UnityWebRequest request = new UnityWebRequest("http://13.209.33.42/api/user/login", "POST");
        request.uploadHandler = new UploadHandlerRaw(bodyRaw);
        request.downloadHandler = new DownloadHandlerBuffer();
        request.SetRequestHeader("Content-Type", "application/json");
    ...
    private IEnumerator SendSignupRequest()
    {
        ...
        UnityWebRequest request = new UnityWebRequest("http://13.209.33.42/api/user/register", "POST");
        request.uploadHandler = new UploadHandlerRaw(bodyRaw);
        request.downloadHandler = new DownloadHandlerBuffer();
        request.SetRequestHeader("Content-Type", "application/json");

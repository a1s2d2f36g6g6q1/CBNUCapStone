...
public class P001_Planet : MonoBehaviour
{
    ...
    {
        StartCoroutine(FetchPlanetList());
    }

    private System.Collections.IEnumerator FetchPlanetList()
    {
        // API: GET /api/planet/explore?sort=recent (또는 popular)
        string sortType = isRecentSort ? "recent" : "popular";

        UnityWebRequest request = UnityWebRequest.Get(
            $"http://13.209.33.42/api/planet/explore?sort={sortType}"
        );
